...
describe('MunderoIntegrationService', () => {
  ...
  describe('verifyAuth', () => {
    it('should verify authentication successfully', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith('🔐 Verifying authentication with Mundero...');
      expect(console.log).toHaveBeenCalledWith('✅ Authentication verified successfully');
    });
  ...
  describe('syncProfile', () => {
    it('should sync profile successfully', async () => {
      ...
      expect(result).toEqual(mockResponse);
      expect(console.log).toHaveBeenCalledWith('🔄 Syncing profile with Mundero...', { userId: 'user123' });
      expect(console.log).toHaveBeenCalledWith('✅ Profile synced successfully');
    });
  ...
  describe('reportReferral', () => {
    it('should report referral successfully', async () => {
      ...
      expect(result).toEqual(mockResponse);
      expect(console.log).toHaveBeenCalledWith('📈 Reporting referral to Mundero...', {
        referrerUserId: 'user123',
        referredEmail: 'referred@example.com',
        source: 'legality360'
      });
    ...
    it('should handle referral report failure', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith('❌ Referral report failed:', expect.any(Error));
    });
  });
  ...
  describe('reportCommission', () => {
    it('should report commission successfully', async () => {
      ...
      expect(result).toEqual(mockResponse);
      expect(console.log).toHaveBeenCalledWith('💰 Reporting commission to Mundero...', {
        userId: 'user123',
        amount: 100.00,
        currency: 'USD',
        type: 'referral',
    ...
    it('should handle commission report failure', async () => {
      ...
      mockApiClient.post.mockRejectedValue(new Error('Invalid commission amount'));

      await expect(service.reportCommission(commissionData)).rejects.toThrow(
        'Commission report failed: Invalid commission amount'
      );

      expect(console.error).toHaveBeenCalledWith('❌ Commission report failed:', expect.any(Error));
    });
  });
  ...
  describe('getHealthStatus', () => {
    it('should get health status successfully', async () => {
      ...
      expect(result).toEqual(mockResponse);
      expect(console.log).toHaveBeenCalledWith('🏥 Checking Mundero integration health...');
      expect(console.log).toHaveBeenCalledWith('✅ Integration health: healthy');
    });
    ...
    it('should handle degraded health status', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith('⚠️ Integration health: degraded');
    });
  });
  ...
  describe('getReferralStats', () => {
    it('should get referral statistics successfully', async () => {
      ...
      expect(result).toEqual(mockResponse);
      expect(console.log).toHaveBeenCalledWith('📊 Getting referral statistics...', { userId: 'user123' });
    });
  });
  ...
  describe('testConnection', () => {
    it('should return true for healthy connection', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith('🔌 Testing Mundero integration connectivity...');
      expect(console.log).toHaveBeenCalledWith('✅ Integration connectivity test passed');
    });
    ...
    it('should return false for unhealthy connection', async () => {
      ...
      expect(console.warn).toHaveBeenCalledWith('⚠️ Integration connectivity test failed - unhealthy status');
    });
    ...
    it('should return false when health check throws error', async () => {
      ...
      expect(result).toBe(false);
      expect(console.error).toHaveBeenCalledWith('❌ Integration connectivity test failed:', expect.any(Error));
    });
  });
});
...
describe('MunderoIntegrationUtils', () => {
  describe('formatCommissionAmount', () => {
    it('should format USD amounts correctly', () => {
      expect(MunderoIntegrationUtils.formatCommissionAmount(100.50, 'USD')).toBe('$100.50');
      expect(MunderoIntegrationUtils.formatCommissionAmount(1000, 'USD')).toBe('$1,000.00');
    });

    it('should format EUR amounts correctly', () => {
      expect(MunderoIntegrationUtils.formatCommissionAmount(100.50, 'EUR')).toBe('€100.50');
    });

    it('should handle different currencies', () => {
      expect(MunderoIntegrationUtils.formatCommissionAmount(100, 'PEN')).toContain('100.00');
      expect(MunderoIntegrationUtils.formatCommissionAmount(100, 'COP')).toContain('100.00');
    });
  });
  ...
  describe('isValidIntegrationSource', () => {
    it('should validate correct integration sources', () => {
      expect(MunderoIntegrationUtils.isValidIntegrationSource('legality360')).toBe(true);
      expect(MunderoIntegrationUtils.isValidIntegrationSource('we-consulting')).toBe(true);
      expect(MunderoIntegrationUtils.isValidIntegrationSource('mundero-crm')).toBe(true);
    });

    it('should reject invalid integration sources', () => {
      expect(MunderoIntegrationUtils.isValidIntegrationSource('invalid-source')).toBe(false);
      expect(MunderoIntegrationUtils.isValidIntegrationSource('')).toBe(false);
      expect(MunderoIntegrationUtils.isValidIntegrationSource('LEGALITY360')).toBe(false);
    });
  });

  describe('getIntegrationDisplayName', () => {
    it('should return correct display names', () => {
      expect(MunderoIntegrationUtils.getIntegrationDisplayName('legality360')).toBe('Legality 360');
      expect(MunderoIntegrationUtils.getIntegrationDisplayName('we-consulting')).toBe('WE Consulting');
      expect(MunderoIntegrationUtils.getIntegrationDisplayName('mundero-crm')).toBe('Mundero CRM');
    });

    it('should return source as-is for unknown sources', () => {
      expect(MunderoIntegrationUtils.getIntegrationDisplayName('unknown-source')).toBe('unknown-source');
    });
  });
});
  ...
  it('should handle complete referral workflow', async () => {
    ...
    mockApiClient.post
      .mockResolvedValueOnce({
        success: true,
        data: { valid: true, userId: 'user123', email: 'test@example.com', roles: ['user'], expiresAt: '2024-12-31T23:59:59Z' }
      })
      // Mock successful referral report
      .mockResolvedValueOnce({
        success: true,
        data: { referralId: 'ref-123', status: 'pending', trackingCode: 'TRK-ABC123', processedAt: '2024-01-01T00:00:00Z' }
      });
    ...
    const referralResult = await munderoIntegrationService.reportReferral(referralData);
  ...
  it('should handle complete commission workflow', async () => {
    ...
    mockApiClient.post.mockResolvedValue({
      ...
      data: {
        ...
        status: 'approved',
        calculatedAmount: 100.00,
        commission: { rate: 0.10, baseAmount: 1000.00, finalAmount: 100.00 },
        processedAt: '2024-01-01T00:00:00Z'
      }
    });
    ...
    const result = await munderoIntegrationService.reportCommission(commissionData);
    expect(result.data.commissionId).toBe('comm-123');
    expect(result.data.status).toBe('approved');
    expect(result.data.calculatedAmount).toBe(100.00);
  });
});