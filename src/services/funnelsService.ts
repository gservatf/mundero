...
class FunnelsService {
    ...
    async trackFunnelView(funnelId: string, userId?: string): Promise<void> {
        try {
            ...
            // Register event
            await this.registerFunnelEvent(
                userId || null,
                'direct',
                'awareness',
                { funnelId, action: 'view' }
            );
        } catch (error) {
    ...
    async trackFunnelSubmission(
        ...
        try {
            ...
            await this.registerFunnelEvent(
                ...
                'direct',
                'conversion',
                { funnelId, submissionId, action: 'submit' }
            );
    ...
    async getFunnelBySlug(orgSlug: string, funnelSlug: string): Promise<Funnel | null> {
        try {
    ...
    async createFunnel(
        ...
        try {
            ...
            await this.validateOrgPermission(organizationId, userId, ['owner', 'admin']);
    ...
    async updateFunnel(
        ...
        try {
            ...
            await this.validateOrgPermission(funnel.organizationId, userId, ['owner', 'admin']);
    ...
    async submitFunnel(
        ...
        try {
            ...
            const docRef = await addDoc(collection(db, 'funnel_submissions'), submissionData);
    ...
    async getFunnelSubmissions(
        ...
        try {
            ...
            await this.validateOrgPermission(funnel.organizationId, userId, ['owner', 'admin', 'member']);
    ...
    async getFunnelMetrics(funnelId: string, userId: string): Promise<FunnelMetrics | null> {
        try {
            const funnel = await this.getFunnel(funnelId);
            if (!funnel) {
                return null;
    ...
    private async slugExists(organizationId: string, slug: string): Promise<boolean> {
        const q = query(
            collection(db, 'funnels'),
            where('organizationId', '==', organizationId),
            where('slug', '==', slug),
    ...
    async validateAndRedirectToSolution(
        ...
        try {
            ...
            const access = await solutionsService.validateSolutionAccess(organizationId, solutionKey);
            ...
            if (!access.hasAccess) {
                ...
                await this.sendSolutionAccessRequestEmail(organizationId, solutionKey, access.reason);
            ...
            return {
                success: true,
                redirectUrl: access.solution?.routeReader || `/solutions/${solutionKey}/start`
            };
        } catch (error) {
    ...
    async processFunnelCompletion(
        ...
        try {
            ...
            if (funnel.settings?.destination === 'redirect' && funnel.settings?.redirectUrl) {
                return {
