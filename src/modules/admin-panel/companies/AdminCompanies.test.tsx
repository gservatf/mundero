...
describe("AdminCompanies Service Integration", () => {
    ...
    it("should search companies case-insensitive", async () => {
      ...
      const mockSnapshot = {
        docs: [
          {
            id: "comp-1",
            data: () => ({ ...mockCompany, name: "Tech Solutions" }),
          },
          {
            id: "comp-2",
            data: () => ({ ...mockCompany, name: "Technology Inc" }),
          },
        ],
      };
  ...
  describe("App Management", () => {
    ...
    it("should unlink specific apps", async () => {
      ...
      const mockDocSnap = {
        ...
        data: () => ({
          ...mockCompany,
          apps: ["legality360", "we-consulting", "analytics-pro"],
        }),
      };
      vi.mocked(getDoc).mockResolvedValue(mockDocSnap as any);
      vi.mocked(updateDoc).mockResolvedValue(undefined as any);
  ...
  describe("Error Handling", () => {
    ...
    it("should handle Firebase errors in search", async () => {
      ...
      await expect(adminCompanyService.searchCompanies("test")).rejects.toThrow(
    ...
    it("should handle pagination errors", async () => {
      ...
      await expect(adminCompanyService.getCompaniesPaged(10)).rejects.toThrow(
...
describe("AdminCompanies Integration Workflow", () => {
  it("should simulate complete company management workflow", async () => {
    ...
    const { addDoc, getDoc, updateDoc, getDocs } = await import(
      "firebase/firestore"
    );
    ...
    const statusCallArgs = vi
      .mocked(updateDoc)
      .mock.calls.find(
        (call) => call[1] && typeof call[1] === "object" && "status" in call[1],
      );
    ...
    // Verify update call
    const updateCallArgs = vi
      .mocked(updateDoc)
      .mock.calls.find(
        (call) => call[1] && typeof call[1] === "object" && "type" in call[1],
      );
