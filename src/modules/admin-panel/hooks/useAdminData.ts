...
import { adminUserService, adminCompanyService, type AdminUser, type AdminCompany } from '../services/adminFirebase';
...
export interface RecentActivity {
    id: string;
    type: 'user_registered' | 'company_created' | 'integration_activated' | 'admin_action';
    description: string;
    timestamp: Date;
    userId?: string;
    userName?: string;
...
export const useAdminData = () => {
    ...
    const setLoadingState = useCallback((operation: keyof Pick<AdminDataState, 'isLoading' | 'isLoadingStats' | 'isLoadingUsers' | 'isLoadingCompanies' | 'isLoadingIntegrations'>, loading: boolean) => {
        setState(prev => ({ ...prev, [operation]: loading }));
    }, []);

    // Update error state for a specific operation
    const setErrorState = useCallback((operation: keyof Pick<AdminDataState, 'error' | 'statsError' | 'usersError' | 'companiesError' | 'integrationsError'>, error: string | null) => {
    ...
    const loadDashboardStats = useCallback(async (): Promise<DashboardStats | null> => {
        ...
        try {
            ...
            const companies = companiesResult.status === 'fulfilled' ? companiesResult.value.companies : [];
            ...
            try {
                const integrationsResponse = await apiClient.get('integrations/status');
    ...
    // Load users
    const loadUsers = useCallback(async (options?: { limit?: number; search?: string }): Promise<AdminUser[]> => {
        if (!isAdmin || !adminProfile) {
            return [];
        }
    ...
    // Load companies
    const loadCompanies = useCallback(async (options?: { limit?: number; search?: string }): Promise<AdminCompany[]> => {
        if (!isAdmin || !adminProfile) {
            return [];
        }
        ...
        try {
            ...
            if (options?.search) {
                // Use search method for searching
                const companies = await adminCompanyService.searchCompanies(options.search, options.limit || 100);
                result = { companies, total: companies.length };
            } else {
                // Use regular getCompanies method with limit
                result = await adminCompanyService.getCompanies(options?.limit || 100);
            }
    ...
    const loadIntegrations = useCallback(async (): Promise<IntegrationStatus[]> => {
        if (!isAdmin || !adminProfile) {
            return [];
        }
        ...
        try {
            ...
            if (response.success && response.data) {
                const integrations: IntegrationStatus[] = response.data.integrations || [];
    ...
    }, [isAdmin, adminProfile, loadDashboardStats, loadRecentActivity, loadIntegrations, setLoadingState, setErrorState]);
    ...
    const syncIntegrations = useCallback(async (): Promise<boolean> => {
        ...
        try {
            ...
            setErrorState('integrationsError', error instanceof Error ? error.message : 'Failed to sync integrations');
            return false;
        }
    ...
    const updateUser = useCallback(async (userId: string, updates: Partial<AdminUser>): Promise<boolean> => {
        ...
        try {
            ...
            if (updates.role) {
                await adminUserService.updateUserRole(userId, updates.role, { uid: adminProfile.uid, email: adminProfile.email });
            }
            if (updates.status) {
                await adminUserService.updateUserStatus(userId, updates.status, { uid: adminProfile.uid, email: adminProfile.email });
            }
            ...
            setErrorState('usersError', error instanceof Error ? error.message : 'Failed to update user');
            return false;
        }
    }, [isAdmin, adminProfile, loadUsers, setErrorState]);

    // Update company
    const updateCompany = useCallback(async (companyId: string, updates: Partial<AdminCompany>): Promise<boolean> => {
        if (!isAdmin || !adminProfile) {
            return false;
        }
        ...
        try {
            ...
            setErrorState('companiesError', error instanceof Error ? error.message : 'Failed to update company');
            return false;
        }
