...
export const MessageBubble: React.FC<MessageBubbleProps> = ({
  ...
  return (
    <div
      className={`flex gap-3 ${messageIsOwn ? "flex-row-reverse" : "flex-row"}`}
    >
      {/* Avatar */}
      ...
      {/* Mensaje */}
      <div
        className={`flex flex-col max-w-xs lg:max-w-md ${messageIsOwn ? "items-end" : "items-start"}`}
      >
        <div
        ...
        {/* Timestamp y estado */}
        <div
          className={`flex items-center gap-1 mt-1 px-2 ${messageIsOwn ? "flex-row-reverse" : "flex-row"}`}
        >
