...
export const CompanyInsights: React.FC<CompanyInsightsProps> = ({
    ...
    const insights = useMemo(() => {
        ...
        const avgEngagementRate = analytics.companyStats.reduce((sum, c) => sum + c.engagementRate, 0) / totalCompanies || 0;
        const totalActiveUsers = analytics.companyStats.reduce((sum, c) => sum + c.activeUsers, 0);
        const avgPostsPerCompany = analytics.companyStats.reduce((sum, c) => sum + c.postCount, 0) / totalCompanies || 0;
    ...
    const engagementDistribution = useMemo(() => {
        ...
        if (others.length > 0) {
            const othersTotal = others.reduce((sum, c) => sum + c.totalLikes + c.totalComments + c.totalShares, 0);
            data.push({
                name: 'Otras empresas',
                value: othersTotal,
                color: '#9CA3AF'
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                                    ...
                                    <div>
                                        <span className="text-gray-600 dark:text-gray-400">Promedio por Post</span>
                                        <div className="font-semibold text-gray-900 dark:text-white">
                ...
                <div style={{ width: '100%', height: 250 }}>
                    <ResponsiveContainer>
                        <BarChart data={companyEngagementData}>
                            ...
                            <Bar dataKey="users" fill={colors.accent} name="Usuarios Activos" />
                        </BarChart>
                    </ResponsiveContainer>
                </div>
            </Card>
                    ...
                    <div style={{ width: '60%', height: 250 }}>
                        <ResponsiveContainer>
                            <PieChart>
                                ...
                                <Tooltip
                                    formatter={(value: number) => [value.toLocaleString('es-ES'), 'Engagement']}
                                />
                            </PieChart>
                        </ResponsiveContainer>
                    </div>
                                ...
                                <div key={index} className="flex items-center justify-between">
                                    ...
                                    <div className="text-right">
                                        ...
                                        <div className="text-xs text-gray-500">
                                            {((item.value / engagementDistribution.reduce((sum, d) => sum + d.value, 0)) * 100).toFixed(1)}%
                                        </div>
                                    </div>
                                </div>
                            ))}
