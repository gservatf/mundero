...
const ActivityHeatmap: React.FC<ActivityHeatmapProps> = ({
  ...
  const [activityMatrix, setActivityMatrix] = useState<ActivityMatrix | null>(
    null,
  );
  const [heatmapData, setHeatmapData] = useState<HeatmapCell[]>([]);
  const [selectedCell, setSelectedCell] = useState<HeatmapCell | null>(null);
  const [viewMode, setViewMode] = useState<
    "activity" | "posts" | "interactions" | "users"
  >("activity");
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  <div>
                    ...
                    <div className="font-medium">
                      {Math.round(selectedCell.activity)}
                    </div>
                  </div>
                  <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t">
                <div className="text-center">
                  <div className="text-2xl font-bold text-blue-600">
                    {Object.values(activityMatrix.totalsByHour).reduce(
                      (a: number, b: any) => a + (b.posts || 0),
                      0,
                    )}
                  </div>
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-green-600">
                    {Object.values(activityMatrix.totalsByHour).reduce(
                      (a: number, b: any) => a + (b.interactions || 0),
                      0,
                    )}
                  </div>
