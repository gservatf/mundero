...
class FeedAnalyticsService {
    ...
    private async getPostsInRange(startDate: Date, endDate: Date): Promise<FeedPost[]> {
        try {
    ...
    private calculateEngagementRate(likes: number, comments: number, shares: number, posts: number): number {
        if (posts === 0) return 0;
    ...
    async getFeedAnalytics(period: AnalyticsPeriod = 'weekly'): Promise<FeedAnalytics> {
        try {
            ...
            const engagementRate = this.calculateEngagementRate(totalLikes, totalComments, totalShares, postCount);

            // Get additional analytics
            const [topUsers, companyStats, dailyActivity, topPosts] = await Promise.all([
    ...
    async getTopUsers(posts?: FeedPost[], limitCount: number = 10): Promise<TopUser[]> {
        try {
            ...
            const topUsers: TopUser[] = Array.from(userStats.entries()).map(([uid, stats]) => {
                const totalEngagement = stats.likes + stats.comments;
                const engagementRate = this.calculateEngagementRate(stats.likes, stats.comments, 0, stats.posts);
    ...
    async getCompanyInsights(posts?: FeedPost[]): Promise<CompanyStats[]> {
        try {
            ...
            const companies: CompanyStats[] = Array.from(companyStats.entries()).map(([companyId, stats]) => {
                ...
                const avgEngagementPerPost = stats.postCount > 0
                    ? Math.round((stats.totalLikes + stats.totalComments + stats.totalShares) / stats.postCount * 100) / 100
                    : 0;
    ...
    async getDailyActivity(posts?: FeedPost[], startDate?: Date, endDate?: Date): Promise<DailyActivity[]> {
        try {
            let postsToAnalyze = posts;
            ...
            if (!postsToAnalyze) {
                ...
                if (dateRange.startDate && dateRange.endDate) {
                    postsToAnalyze = await this.getPostsInRange(dateRange.startDate, dateRange.endDate);
                } else {
                    postsToAnalyze = [];
                }
            }
            ...
            const dailyActivity: DailyActivity[] = Array.from(dailyStats.entries()).map(([date, stats]) => ({
                date,
                posts: stats.posts,
                likes: stats.likes,
                comments: stats.comments,
    ...
    async getTopPosts(posts?: FeedPost[], limitCount: number = 10): Promise<TopPost[]> {
        try {
            let postsToAnalyze = posts;
