...
export class SecurityValidator {
  ...
  static validateEmbedUrl(
    ...
    try {
      ...
      const isAllowed = cfg.allowedEmbedDomains.some(
  ...
  private static validateYouTubeUrl(url: string): {
    valid: boolean;
    error?: string;
  } {
    const youtubeRegex =
  ...
  private static validateGoogleDriveUrl(url: string): {
    valid: boolean;
    error?: string;
  } {
    const driveRegex =
      /^https:\/\/(?:drive|docs)\.google\.com\/(?:file\/d\/|document\/d\/|presentation\/d\/|spreadsheets\/d\/)([a-zA-Z0-9-_]+)/;
