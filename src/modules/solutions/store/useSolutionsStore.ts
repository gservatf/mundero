    ...
    (set, get) => ({
        ...
        try {
          ...
          set({
            error:
              error instanceof Error
                ? error.message
                : "Failed to fetch organization solutions",
            loading: false,
          });
          throw error;
        }
      ...
      createSolution: async (data: CreateSolutionData): Promise<Solution> => {
        ...
        try {
          ...
          const solution = await solutionsService.createSolution(
            data,
            createdBy,
          );
        ...
        try {
          ...
          set({
            error:
              error instanceof Error
                ? error.message
                : "Failed to grant solution access",
            loading: false,
          });
          throw error;
        }
      ...
      revokeSolutionAccess: async (orgId: string, solutionKey: string) => {
        ...
        try {
          ...
          const filteredOrgSolutions = orgSolutions.filter(
            (orgSolution) =>
              !(
                orgSolution.orgId === orgId &&
                orgSolution.solutionKey === solutionKey
              ),
          );
          ...
          set({
            error:
              error instanceof Error
                ? error.message
                : "Failed to revoke solution access",
            loading: false,
          });
          throw error;
        }
      ...
      validateAccess: async (
        ...
        try {
          return await solutionsService.validateSolutionAccess(
            orgId,
            solutionKey,
          );
        } catch (error) {
