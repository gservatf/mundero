...
class SolutionsService {
    ...
    async createSolution(data: CreateSolutionData, createdBy: string): Promise<Solution> {
        try {
    ...
    async grantSolutionAccess(data: CreateOrgSolutionData, grantedBy: string): Promise<void> {
        try {
    ...
    async revokeSolutionAccess(orgId: string, solutionKey: string): Promise<void> {
        try {
            const q = query(
                this.orgSolutionsCollection,
                where('orgId', '==', orgId),
    ...
    async updateOrgSolutionSettings(orgId: string, solutionKey: string, settings: Record<string, any>): Promise<void> {
        try {
            const q = query(
                this.orgSolutionsCollection,
                where('orgId', '==', orgId),
    ...
    // Access Validation
    async validateSolutionAccess(orgId: string, solutionKey: string): Promise<SolutionAccess> {
        try {
            ...
            if (!orgSolution.enabled) {
                return { hasAccess: false, solution, orgSolution, reason: 'not_enabled' };
            }
    ...
    // Event Tracking
    async trackSolutionEvent(eventData: Omit<SolutionEvent, 'id' | 'timestamp'>): Promise<void> {
        try {
            const event = {
                ...eventData,
            ...
            if (eventData.event === 'view' || eventData.event === 'conversion') {
                await this.updateUsageStats(eventData.orgId, eventData.solutionKey, eventData.event);
            }
        } catch (error) {
    ...
    private async updateUsageStats(orgId: string, solutionKey: string, event: 'view' | 'conversion'): Promise<void> {
        try {
            const q = query(
                this.orgSolutionsCollection,
                where('orgId', '==', orgId),
            ...
            if (!snapshot.empty) {
                ...
                const usage = currentData.usage || { totalViews: 0, totalConversions: 0 };
    ...
    // Analytics
    async getSolutionEvents(solutionKey: string, orgId?: string, limit = 100): Promise<SolutionEvent[]> {
        try {
            let q = query(
