      ...
      const workflowData: Partial<Workflow> = {
        ...
        nodes: reactFlowNodes.map((node: Node) => ({
          ...
          type: (node.type?.toUpperCase() ||
            "CONDITION") as WorkflowNode["type"],
          position: node.position,
          data: node.data,
        })),
            ...
            >
              <div className="flex items-center gap-2 text-sm text-gray-600">
                ...
                <Badge variant="outline">
                  {reactFlowEdges.length} conexiones
                </Badge>
              </div>
            </Panel>
          </ReactFlow>
...
function EmailNode({ data, selected }: { data: any; selected: boolean }) {
  return (
    <Card
      className={`p-4 min-w-[200px] ${selected ? "ring-2 ring-blue-500" : ""}`}
    >
      ...
      <p className="text-sm text-gray-600">
        {data.config?.subject || "Sin asunto"}
      </p>
    </Card>
  );
}
...
function WebhookNode({ data, selected }: { data: any; selected: boolean }) {
  return (
    ...
    >
      ...
      <p className="text-sm text-gray-600">
        {data.config?.url || "URL no configurada"}
      </p>
    </Card>
  );
}
...
function NotifyHrNode({ data, selected }: { data: any; selected: boolean }) {
  return (
    ...
    >
      ...
      <p className="text-sm text-gray-600">
        {data.config?.message || "Sin mensaje"}
      </p>
    </Card>
  );
}

function StartSolutionNode({
  data,
  selected,
}: {
  data: any;
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-gray-600">
        {data.config?.solutionId || "Sin solución"}
      </p>
    </Card>
  );
}
