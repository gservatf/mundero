...
export const FunnelDashboard: React.FC<FunnelDashboardProps> = ({
  ...
  const loadFunnels = async () => {
    try {
      setLoading(true);
      const orgFunnels = await funnelsService.getOrganizationFunnels(
        organization.id,
      );
  ...
  const filteredFunnels = funnels.filter((funnel) => {
    ...
    const matchesStatus =
      statusFilter === "all" || funnel.status === statusFilter;
  ...
  const totalViews = funnels.reduce(
    (sum, funnel) => sum + funnel.analytics.views,
    0,
  );
  const totalSubmissions = funnels.reduce(
    (sum, funnel) => sum + funnel.analytics.submissions,
    0,
  );
  const avgConversionRate =
    funnels.length > 0
      ? funnels.reduce(
          (sum, funnel) => sum + funnel.analytics.conversionRate,
          0,
        ) / funnels.length
      : 0;
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600">
            Gestiona los embudos de conversión de {organization.name}
          </p>
        </div>
      ...
      {filteredFunnels.length === 0 ? (
        <Card className="p-8 text-center">
          <div className="max-w-md mx-auto">
            ...
            {!searchTerm && statusFilter === "all" && (
              <Button onClick={onCreateFunnel}>Crear mi primer funnel</Button>
            )}
          </div>
        </Card>
      ) : (
              ...
              <Card className="p-6 hover:shadow-lg transition-shadow">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge
                      className={`${getStatusColor(funnel.status)} text-white text-xs`}
                    >
