...
export const FunnelMetrics: React.FC<FunnelMetricsProps> = ({
  ...
  const loadMetrics = async () => {
    try {
      setLoading(true);
      const metricsData = await funnelsService.getFunnelMetrics(
        funnel.id,
        "current-user-id",
      );
  ...
  const stepAnalysis = funnel.steps.map((step, index) => ({
    ...
    dropOff:
      ((Math.max(100 - index * 15, 20) - Math.max(85 - index * 12, 15)) /
        Math.max(100 - index * 15, 20)) *
      100,
  }));
  ...
  const generateSolutionChartData = () => {
    const last7Days = Array.from({ length: 7 }, (_, i) => {
      ...
      const dayEvents = solutionEvents.filter(
        ...
        <Card className="p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">
                {metrics?.submissions?.toLocaleString() ||
        ...
        <Card className="p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">
                {(
                  metrics?.conversionRate || funnel.analytics.conversionRate
                ).toFixed(1)}
                %
              </p>
          ...
          <Card className="p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-900">
                Métricas de Solución Empresarial
              </h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="text-center p-4 bg-blue-50 rounded-lg">
                ...
                <p className="text-sm text-gray-600">
                  Redireccionado a Solución
                </p>
              </div>
              <div className="text-center p-4 bg-green-50 rounded-lg">
                <p className="text-2xl font-bold text-green-600">
                  {
                    solutionEvents.filter((e) => e.event === "access_granted")
                      .length
                  }
                </p>
              ...
              <div className="text-center p-4 bg-purple-50 rounded-lg">
                <p className="text-2xl font-bold text-purple-600">
                  {
                    solutionEvents.filter((e) => e.event === "conversion")
                      .length
                  }
                </p>
                <p className="text-sm text-gray-600">
                ...
                <p className="text-2xl font-bold text-orange-600">
                  {solutionEvents.length > 0
                    ? (
                        (solutionEvents.filter((e) => e.event === "conversion")
                          .length /
                          solutionEvents.filter((e) => e.event === "redirect")
                            .length) *
                        100
                      ).toFixed(1)
                    : "0.0"}
            ...
            {solutionEvents.length > 0 && (
              <div className="mt-6">
                <h4 className="text-md font-medium text-gray-900 mb-3">
                  Actividad en Solución (Últimos 7 días)
                </h4>
        ...
        <TabsContent value="funnel" className="space-y-6">
          <Card className="p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <p className="text-sm text-gray-600">
                Identifica dónde pierdes más usuarios
              </p>
            </div>
              ...
              {stepAnalysis.map((step, index) => (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <Badge
                      variant={step.dropOff > 30 ? "destructive" : "default"}
                    >
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div
                      ...
                      style={{
                        width: `${(step.completions / step.views) * 100}%`,
                      }}
                    ></div>
                  </div>
                </div>
                  ...
                  >
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs text-gray-600">
                        {Math.round(
                          (funnel.analytics.views * source.value) / 100,
                        )}{" "}
                        vistas
                      </p>
                    </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center p-3 bg-green-50 rounded-lg">
                    ...
                    <span className="font-bold text-green-600">
                      {
                        solutionEvents.filter(
                          (e) => e.event === "access_granted",
                        ).length
                      }
                    </span>
                  </div>
              ...
              <div className="overflow-x-auto">
                ...
                {solutionEvents.length === 0 && (
                  <div className="text-center py-8">
                    <p className="text-gray-600">
                      No hay eventos de solución registrados
                    </p>
                  </div>
                )}
              </div>
