...
class StatsService {
  ...
  private async calculateGlobalStats(): Promise<ReputationStats> {
    ...
    const [
      ...
      getAggregateFromServer(reputationRef, {
    ...
    const activeUsersWithReputation = allReputationSnapshot.docs.filter(
      (doc) => doc.data().totalPoints > 0,
    ).length;
  ...
  private calculateLevelDistribution(
    docs: any[],
  ): { level: number; count: number; percentage: number }[] {
  ...
  async getTopUsersByCategory(
    category: "points" | "level" | "badges" = "points",
    ...
    try {
      ...
      const snapshot = await getDocs(
        query(reputationRef, orderBy(orderField, "desc"), limit(limitCount)),
      );
