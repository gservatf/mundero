      ...
      setUsers((prev) =>
        prev.map((u) =>
          u.userId === editingUser
            ? {
                ...u,
                totalPoints: tempChanges.points,
                badges: tempChanges.badges,
              }
            : u,
        ),
      );
      ...
      <div className="max-w-6xl mx-auto p-6">
        <div className="bg-gray-100 dark:bg-gray-800 rounded-xl p-8 text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400">
            El panel administrativo estará disponible cuando se active el
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <LevelChip
                        level={user.level}
                        name={
                          DEFAULT_LEVELS.find((l) => l.id === user.level)
                            ?.name || "Explorador"
                        }
                        size="small"
                      />
                  ...
                  {adminActions
                    ...
                    .map((action) => (
                      ...
                      >
                        <div>
                          <div className="text-sm font-medium text-gray-900 dark:text-white">
                            {action.action === "points_add"
                              ? "➕ Puntos añadidos"
                              : action.action === "points_subtract"
                                ? "➖ Puntos sustraídos"
                                : action.action === "badge_add"
                                  ? "🏆 Insignia añadida"
                                  : action.action === "badge_remove"
                                    ? "🚫 Insignia removida"
                                    : "📝 Nivel cambiado"}
                          </div>
                          <div className="text-xs text-gray-500 dark:text-gray-400">
                            Usuario: {action.userId} • Valor: {action.value} •{" "}
                            {action.reason}
                          </div>
                        </div>
                        <div className="text-xs text-gray-500 dark:text-gray-400">
