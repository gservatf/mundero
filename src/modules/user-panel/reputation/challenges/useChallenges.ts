...
import { Challenge, ChallengeTemplate, ChallengeStatus, CHALLENGE_TEMPLATES } from './types';
...
class ChallengeService {
        ...
        this.challenges = [
            {
                ...
                description: 'Expande tu red profesional conectando con 3 personas nuevas en la plataforma',
                type: 'individual',
                points: 100,
                creatorId: 'system',
                ...
                progress: {
                    ...
                    user2: { current: 3, completed: true, completedAt: now - 60 * 60 * 1000 },
            ...
            },
            {
                ...
                description: 'Entre todos los participantes, alcanzar 1000 likes en publicaciones nuevas',
                type: 'collaborative',
                points: 150,
                creatorId: 'user1',
    ...
    async createChallenge(template: ChallengeTemplate, creatorId: string): Promise<Challenge> {
    ...
    async updateProgress(challengeId: string, userId: string, newProgress: number): Promise<void> {
...
export const useChallenges = () => {
    ...
    const loadChallenges = useCallback(async (status?: ChallengeStatus) => {
        try {
            setLoading(true);
            setError(null);
    ...
    // Crear nuevo reto
    const createChallenge = useCallback(async (template: ChallengeTemplate): Promise<Challenge | null> => {
        if (!user) {
        ...
        try {
            const challenge = await challengeService.createChallenge(template, user.id);
    ...
    const joinChallenge = useCallback(async (challengeId: string): Promise<boolean> => {
        ...
        try {
            const success = await challengeService.joinChallenge(challengeId, user.id);
    ...
    const leaveChallenge = useCallback(async (challengeId: string): Promise<boolean> => {
        ...
        try {
            const success = await challengeService.leaveChallenge(challengeId, user.id);
    ...
    const updateProgress = useCallback(async (challengeId: string, newProgress: number): Promise<void> => {
        if (!user) return;

        try {
            await challengeService.updateProgress(challengeId, user.id, newProgress);
        } catch (err) {
    ...
    const getUserProgress = useCallback((challengeId: string): { current: number; target: number; percentage: number; completed: boolean } => {
        if (!user) {
