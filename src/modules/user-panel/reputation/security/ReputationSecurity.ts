...
class ReputationSecurityManager {
    ...
    async validateReputationAction(
        ...
        try {
            ...
            const fraudResult = this.detectFraudPatterns(userId, action, userHistory, metadata);
            if (fraudResult.isSuspicious) {
                issues.push(...fraudResult.reasons);
                riskLevel = fraudResult.confidence > 80 ? 'critical' :
                    fraudResult.confidence > 60 ? 'high' : 'medium';
            }

            // 4. Validar límites por hora
            const hourlyValidation = this.validateHourlyLimits(userId, action, userHistory);
            ...
            if (!metadataValidation.isValid) {
                ...
                riskLevel = Math.max(riskLevel === 'low' ? 0 : riskLevel === 'medium' ? 1 : riskLevel === 'high' ? 2 : 3,
                    metadataValidation.riskLevel === 'low' ? 0 : metadataValidation.riskLevel === 'medium' ? 1 : 2) === 0 ? 'low' :
                    Math.max(riskLevel === 'low' ? 0 : riskLevel === 'medium' ? 1 : riskLevel === 'high' ? 2 : 3,
                        metadataValidation.riskLevel === 'low' ? 0 : metadataValidation.riskLevel === 'medium' ? 1 : 2) === 1 ? 'medium' : 'high';
            }
    ...
    private detectFraudPatterns(
        ...
        const sameActionCount = recentActions.filter(log => log.action === action).length;
        if (sameActionCount >= this.securityConfig.suspiciousActionThreshold) {
            reasons.push(`Demasiadas acciones del tipo "${action}" en la última hora (${sameActionCount})`);
            confidence += 30;
        }
        ...
        if (recentActions.length >= 20) {
            ...
            if (avgInterval < 5000) { // menos de 5 segundos entre acciones
        ...
        if (metadata) {
            const metadataPattern = this.analyzeMetadataPattern(userId, action, metadata, history);
    ...
    private validateHourlyLimits(
        ...
        if (actionCount >= this.securityConfig.maxActionsPerHour) {
            issues.push(`Límite de acciones por hora excedido (${actionCount}/${this.securityConfig.maxActionsPerHour})`);
        }

        // Contar puntos ganados en la última hora
        const pointsGained = recentActions.reduce((sum, log) => sum + log.points, 0);
        if (pointsGained >= this.securityConfig.maxPointsPerHour) {
            issues.push(`Límite de puntos por hora excedido (${pointsGained}/${this.securityConfig.maxPointsPerHour})`);
        }
    ...
    private analyzeMetadataPattern(
        ...
        const recentSameAction = history
            .filter(log => log.action === action && log.createdAt >= Date.now() - 30 * 60 * 1000)
            .slice(0, 10);
    ...
    private async getUserRecentHistory(userId: string): Promise<ReputationLog[]> {
        try {
            ...
            if (cached) {
                return cached.filter(log => log.createdAt >= Date.now() - 24 * 60 * 60 * 1000);
            }
            return [];
        } catch (error) {
    ...
    private isValidActionType(action: string): action is ReputationActionType {
        const validActions: ReputationActionType[] = [
            ...
            'community_join', 'community_create', 'event_attend',
            'profile_complete', 'referral_approved'
        ];
    ...
    private async flagForReview(
        ...
        try {
            console.warn(`⚠️ Acción marcada para revisión - Usuario: ${userId}, Acción: ${action}`);
    ...
    private async performSecuritySync(): Promise<SyncStatus> {
        ...
        try {
            ...
            console.log(`✅ Sincronización completada en ${Date.now() - syncStart}ms`);
    ...
    private async cleanupHistoryCache(): Promise<void> {
        ...
        for (const [userId, history] of this.userActionHistory.entries()) {
            const recentHistory = history.filter(log => log.createdAt >= cutoffTime);
    ...
    async forceUserValidation(userId: string): Promise<SecurityValidation> {
        ...
        return this.validateReputationAction(userId, lastAction.action, lastAction.meta);
    }
}
...
export const validateReputationAction = (
    ...
    return reputationSecurityManager.validateReputationAction(userId, action, metadata);
};
