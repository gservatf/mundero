...
export const ActivityLog: React.FC<ActivityLogProps> = ({
    ...
    try {
      ...
      if (filters.dateRange !== "all") {
        filteredLogs = filteredLogs.filter(
          (log) => log.createdAt >= dateFilters[filters.dateRange],
        );
      }
      ...
      if (filters.actionType !== "all") {
        filteredLogs = filteredLogs.filter(
          (log) => log.action === filters.actionType,
        );
      }
      ...
      if (filters.minPoints) {
        filteredLogs = filteredLogs.filter(
          (log) => log.points >= filters.minPoints!,
        );
      }
      if (filters.maxPoints) {
        filteredLogs = filteredLogs.filter(
          (log) => log.points <= filters.maxPoints!,
        );
      }
  ...
  const stats = useMemo((): ActivityStats => {
    ...
    const topAction =
      (Object.keys(actionCounts).reduce(
    ...
    while (streakDays < 30) {
      ...
      const hasActivity = logs.some(
        (log) =>
          log.createdAt >= dayStart.getTime() &&
          log.createdAt <= dayEnd.getTime(),
      );
    ...
    const lastWeekLogs = logs.filter(
      (log) => log.createdAt >= twoWeeksAgo && log.createdAt < oneWeekAgo,
    );
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute(
      "download",
      `actividad_reputacion_${new Date().toISOString().split("T")[0]}.csv`,
    );
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-gray-600">
              Historial completo de acciones y puntos ganados
            </p>
          </div>
