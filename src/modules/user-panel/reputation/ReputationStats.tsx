...
export const ReputationStatsComponent: React.FC = () => {
    ...
    const getLevelName = (level: number): string => {
        const names = ['', 'Explorador', 'Colaborador', 'Pro', 'Embajador', 'Líder'];
            ...
            <div className="max-w-6xl mx-auto p-6">
                <div className="bg-gray-100 dark:bg-gray-800 rounded-xl p-8 text-center">
                    ...
                    <p className="text-gray-500 dark:text-gray-400">
                        Las estadísticas estarán disponibles cuando se active el sistema de reputación.
                    </p>
                </div>
            </div>
        );
        ...
        <div className="max-w-6xl mx-auto p-6 space-y-6">
            ...
            >
                <div>
                    ...
                    <p className="text-gray-600 dark:text-gray-400 mt-2">
                        Datos globales del sistema • Actualizado: {new Date(stats.lastUpdated).toLocaleString()}
                    </p>
                </div>
            ...
            >
                ...
                <div className="bg-white dark:bg-neutral-800 rounded-xl p-6 border border-gray-200 dark:border-neutral-700">
                    <div className="flex items-center justify-between">
                        <div>
                            <p className="text-sm text-gray-600 dark:text-gray-400">Nivel Promedio</p>
                            <p className="text-2xl font-bold text-gray-900 dark:text-white">
                                {stats.averageLevel}
                            </p>
                        </div>
                        ...
                        {stats.topLevelDistribution.map((item) => (
                            <div key={item.level} className="flex items-center justify-between">
                                <div className="flex items-center space-x-2">
                                    <div className={`w-3 h-3 rounded-full bg-gradient-to-r ${item.level === 1 ? 'from-gray-400 to-gray-500' :
                                        item.level === 2 ? 'from-green-400 to-green-500' :
                                            item.level === 3 ? 'from-blue-400 to-blue-500' :
                                                item.level === 4 ? 'from-purple-400 to-purple-500' :
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
                    <div>
                        ...
                        <div className="text-lg font-semibold text-gray-900 dark:text-white">
                            {stats.totalUsers > 0 ? Math.round((stats.activeUsersWithReputation / stats.totalUsers) * 100) : 0}%
                        </div>
                    ...
                    <div>
                        ...
                        <div className="text-lg font-semibold text-gray-900 dark:text-white">
                            {stats.activeUsersWithReputation > 0 ?
                                Math.round((stats.totalBadgesAwarded / stats.activeUsersWithReputation) * 10) / 10 : 0}
                        </div>
                        <div className="text-xs text-gray-500 dark:text-gray-400">
                            Por usuario activo
                        </div>
                    ...
                    <div>
                        ...
                        <div className="text-lg font-semibold text-gray-900 dark:text-white">
                            {stats.activeUsersWithReputation > 0 ?
                                Math.round(stats.totalPointsInSystem / stats.activeUsersWithReputation) : 0}
                        </div>
                        <div className="text-xs text-gray-500 dark:text-gray-400">
                            Por usuario activo
                        </div>
