...
export const RedemptionHistory: React.FC<RedemptionHistoryProps> = ({
    ...
    const getDeliveryIcon = (method: string) => {
        switch (method) {
            ...
            case 'physical': return <FiPackage className="w-4 h-4" />;
            case 'instant': return <FiZap className="w-4 h-4" />;
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between space-y-4 lg:space-y-0">
                <div>
                    <h2 className="text-xl font-semibold text-gray-900">Historial de Canjes</h2>
                    <p className="text-gray-600">Gestiona y revisa tus recompensas canjeadas</p>
                </div>
                ...
                <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg p-4">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-2xl font-bold text-blue-900">{redemptions.length}</p>
                        </div>
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-2xl font-bold text-purple-900">
                                {redemptions.reduce((sum, r) => sum + r.pointsSpent, 0).toLocaleString()}
                            </p>
                        </div>
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center space-x-4">
                                            ...
                                            {redemption.deliveryInfo && (
                                                <div className="flex items-center space-x-1 text-xs text-gray-600">
                                                    ...
                                                    <span className="capitalize">{redemption.deliveryInfo.method}</span>
                                                </div>
                                            )}

                                            {redemption.redemptionCode && (
                                                <div className="text-xs text-gray-600">
                                                    Código: <span className="font-mono font-medium">{redemption.redemptionCode}</span>
                                                </div>
                                            )}
                                        </div>
                                        ...
                                        {redemption.status === 'delivered' && redemption.deliveredAt && (
                                            <div className="text-xs text-green-600">
                                                Entregado el {new Date(redemption.deliveredAt).toLocaleDateString()}
                                            </div>
                                        )}
                                    </div>

                                    {/* Expiration */}
                                    {redemption.expiresAt && redemption.status !== 'delivered' && (
                                        <div className="mt-2 text-xs text-orange-600">
                                            Expira el {new Date(redemption.expiresAt).toLocaleDateString()}
                                        </div>
                                    )}
                                    ...
                                    {redemption.deliveryInfo?.trackingNumber && (
                                        <div className="mt-2 text-xs text-blue-600">
                                            Seguimiento: <span className="font-mono">{redemption.deliveryInfo.trackingNumber}</span>
                                        </div>
                                    )}
                                </div>
                            </div>
