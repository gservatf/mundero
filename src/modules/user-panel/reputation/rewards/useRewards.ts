...
export const useRewards = (userId: string): UseRewardsReturn => {
    ...
    useEffect(() => {
        const loadRewards = async () => {
            try {
                ...
                const mockRewards: Reward[] = Object.entries(REWARD_TEMPLATES).map(([key, template], index) => ({
                    ...template,
                    id: `reward_${key}`,
    ...
    const redeemReward = useCallback(async (rewardId: string): Promise<boolean> => {
        try {
            ...
            setUserStats(prev => prev ? {
                ...
                recentRedemptions: [newRedemption, ...prev.recentRedemptions.slice(0, 4)]
            } : null);
            ...
            if (reward.type === 'premium_features' || reward.type === 'badge') {
                setTimeout(() => {
                    setUserRedemptions(prev => prev.map(redemption =>
                        redemption.id === newRedemption.id
                            ? { ...redemption, status: 'delivered', deliveredAt: Date.now() }
                            : redemption
                    ));
                }, 2000);
            }
    ...
    const checkRedemptionStatus = useCallback(async (redemptionId: string): Promise<RedemptionStatus> => {
        const redemption = userRedemptions.find(r => r.id === redemptionId);
        return redemption?.status || 'pending';
    }, [userRedemptions]);

    // Obtener historial de canjes
    const getUserRedemptionHistory = useCallback(async (): Promise<RewardRedemption[]> => {
    ...
    const getRewardsByCategory = useCallback((category: RewardCategory): Reward[] => {
        return rewards.filter(reward => reward.category === category);
    }, [rewards]);

    // Verificar si el usuario puede canjear una recompensa
    const canUserRedeem = useCallback((rewardId: string): { canRedeem: boolean; reason?: string } => {
        const reward = rewards.find(r => r.id === rewardId);
        if (!reward) {
        ...
        if (reward.availability.expiresAt && reward.availability.expiresAt < Date.now()) {
