...
import { type UserRewardStats as UserStatsType, Reward, RewardCategory } from './types';
...
export const UserRewardStatsComponent: React.FC<UserRewardStatsComponentProps> = ({
    userId,
    stats,
    rewards
}) => {
    ...
    const categoryStats = stats.recentRedemptions.reduce((acc, redemption) => {
    ...
    const totalRedemptions = Object.values(categoryStats).reduce((sum, cat) => sum + cat.count, 0);
    const averagePointsPerRedemption = stats.totalPointsSpent / stats.totalRedemptions;
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg p-4">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-2xl font-bold text-blue-900">{stats.totalRedemptions}</p>
                        </div>
                ...
                <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg p-4">
                    <div className="flex items-center justify-between">
                        <div>
                            <p className="text-sm text-purple-600 font-medium">Puntos Gastados</p>
                            <p className="text-2xl font-bold text-purple-900">{stats.totalPointsSpent.toLocaleString()}</p>
                        </div>
                ...
                <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-lg p-4">
                    <div className="flex items-center justify-between">
                        <div>
                            <p className="text-sm text-green-600 font-medium">Promedio por Canje</p>
                            <p className="text-2xl font-bold text-green-900">{Math.round(averagePointsPerRedemption).toLocaleString()}</p>
                        </div>
                ...
                <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-lg p-4">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-2xl font-bold text-orange-900">{stats.availablePoints.toLocaleString()}</p>
                        </div>
                ...
                <div className="bg-white rounded-lg p-6 border border-gray-200">
                    ...
                    <div className="space-y-4">
                        {Object.entries(categoryStats).map(([category, data]) => {
                            const percentage = totalRedemptions > 0 ? (data.count / totalRedemptions) * 100 : 0;

                            return (
                                <div key={category} className="space-y-2">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center space-x-2">
                                            <span className="text-lg">{getCategoryIcon(category as RewardCategory)}</span>
                                            <span className="text-sm font-medium text-gray-700 capitalize">{category}</span>
                                        </div>
                                        <div className="text-right">
                                            <div className="text-sm font-semibold text-gray-900">{data.count} canjes</div>
                                            <div className="text-xs text-gray-500">{data.points.toLocaleString()} pts</div>
                                        </div>
                                    </div>
                                ...
                                <div key={month.month} className="flex-1 flex flex-col items-center">
                                    ...
                                    >
                                        <span className="text-xs text-white font-medium mb-1">
                                            {month.points > maxMonthlyPoints * 0.7 ? month.points.toLocaleString() : ''}
                                        </span>
                                    </motion.div>
            ...
            <div className="bg-white rounded-lg p-6 border border-gray-200">
                ...
                <div className="mb-4 p-4 bg-purple-50 rounded-lg">
                    <div className="flex items-center space-x-2 mb-2">
                        <span className="text-lg">{getCategoryIcon(stats.favoriteCategory)}</span>
                        <span className="text-sm font-medium text-purple-800">
                            Tu categoría favorita: <span className="capitalize">{stats.favoriteCategory}</span>
                        </span>
                    </div>
                        ...
                        {recommendedRewards.map((reward, index) => (
                            ...
                            >
                                <div className="flex items-center space-x-3 mb-3">
                                    <div className={`w-10 h-10 bg-gradient-to-br ${getCategoryColor(reward.category)} rounded-lg flex items-center justify-center`}>
                                        {reward.icon}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <h4 className="font-medium text-gray-900 truncate">{reward.title}</h4>
