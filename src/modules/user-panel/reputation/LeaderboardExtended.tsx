...
export type LeaderboardCategory = 'global' | 'empresa' | 'comunidad' | 'semanal' | 'challenges' | 'duels';
...
export const LeaderboardExtended: React.FC<LeaderboardExtendedProps> = ({
        ...
        const generateMockUsers = (): LeaderboardUser[] => {
            ...
            for (let i = 4; i <= 20; i++) {
                ...
                mockUsers.push({
                    ...
                    rank: i > 7 ? i : i + 1,
                    previousRank: i > 7 ? i + Math.floor(Math.random() * 3) - 1 : i + 1 + Math.floor(Math.random() * 3) - 1,
    ...
    const categories = [
        ...
        { id: 'comunidad', name: 'Comunidad', icon: FiStar, disabled: !communityId },
                ...
                <div className="flex flex-wrap gap-2">
                    {categories.map((category) => {
                        ...
                        return (
                            <button
                                key={category.id}
                                onClick={() => !category.disabled && setActiveCategory(category.id as LeaderboardCategory)}
                        ...
                        <div className="flex items-center space-x-4">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {periods.map((period) => (
                                    <button
                                        key={period.id}
                                        onClick={() => setActivePeriod(period.id as LeaderboardPeriod)}
                                        className={`
                                ...
                                <div className="flex items-center space-x-3 flex-1 min-w-0">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center space-x-2">
                                            <h3 className={`font-semibold text-gray-900 truncate ${user.id === currentUserId ? 'text-purple-700' : ''}`}>
                                                {user.name}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                    ...
                    <div>
                        <div className="text-2xl font-bold text-blue-600">
                            {Math.round(filteredUsers.reduce((sum, user) => sum + user.totalPoints, 0) / filteredUsers.length).toLocaleString()}
                        </div>
                        <div className="text-sm text-gray-600">Promedio</div>
                    </div>
                    <div>
                        <div className="text-2xl font-bold text-green-600">
                            {Math.max(...filteredUsers.map(user => user.totalPoints)).toLocaleString()}
                        </div>
                        <div className="text-sm text-gray-600">Más alto</div>
                    </div>
                    <div>
                        <div className="text-2xl font-bold text-orange-600">
                            {filteredUsers.reduce((sum, user) => sum + user.weeklyPoints, 0).toLocaleString()}
                        </div>
