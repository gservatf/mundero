...
export const DuelProgress: React.FC<DuelProgressProps> = ({
    ...
    const maxProgress = Math.max(challenger?.current || 0, opponent?.current || 0, 1);
    const challengerPercentage = getProgressPercentage(challenger?.current || 0, maxProgress);
    const opponentPercentage = getProgressPercentage(opponent?.current || 0, maxProgress);
    ...
    const getLeadIndicator = () => {
        ...
        if (challengerScore === opponentScore) {
            return { type: 'tie', message: '⚖️ Empate perfecto', color: 'text-gray-600' };
        }
    ...
    const getVelocityIndicator = (milestones: number[]) => {
        ...
        const velocity = recent.length > 1 ? recent[recent.length - 1] - recent[0] : 0;

        if (velocity > 5) return { icon: '🚀', text: 'Acelerando', color: 'text-green-600' };
        if (velocity > 2) return { icon: '⚡', text: 'Ritmo alto', color: 'text-yellow-600' };
        if (velocity > 0) return { icon: '📈', text: 'Progreso', color: 'text-blue-600' };
        if (velocity === 0) return { icon: '⏸️', text: 'Sin cambios', color: 'text-gray-600' };
    ...
    const getPredictedWinner = () => {
        ...
        const challengerVelocity = challenger.milestones.length > 1 ?
            (challenger.current - challenger.milestones[0]) / challenger.milestones.length : 0;
        const opponentVelocity = opponent.milestones.length > 1 ?
            (opponent.current - opponent.milestones[0]) / opponent.milestones.length : 0;

        if (Math.abs(challengerVelocity - opponentVelocity) < 0.1) {
            return { prediction: 'tie', confidence: 50 };
        }

        const winner = challengerVelocity > opponentVelocity ? 'challenger' : 'opponent';
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="text-center">
                <div className={`inline-flex items-center px-4 py-2 rounded-full ${leadIndicator.color} bg-opacity-10 border border-current border-opacity-20`}>
            ...
            {/* Tiempo restante */}
            <div className="text-center">
                <div className={`text-2xl font-bold ${timeRemaining.hours < 1 ? 'text-red-600' :
                        timeRemaining.hours < 6 ? 'text-yellow-600' : 'text-green-600'
                    }`}>
                ...
                <div className="w-full bg-gray-200 rounded-full h-2 mt-2">
                    <motion.div
                        ...
                        className={`h-2 rounded-full ${timeRemaining.percentage > 75 ? 'bg-red-500' :
                                timeRemaining.percentage > 50 ? 'bg-yellow-500' : 'bg-green-500'
                            }`}
                    />
                </div>
            </div>
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-3">
                            ...
                            <div>
                                ...
                                {challengerVelocity && (
                                    <div className={`text-xs flex items-center ${challengerVelocity.color}`}>
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-3">
                            ...
                            <div>
                                ...
                                {opponentVelocity && (
                                    <div className={`text-xs flex items-center ${opponentVelocity.color}`}>
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                            <div className="font-bold text-lg">
                                {prediction.prediction === 'tie' ? 'Empate' :
                                    prediction.prediction === 'challenger' ? 'Retador' : 'Oponente'}
                            </div>
                            <div className="text-sm text-gray-600">
                            ...
                            <div className="flex justify-between">
                                ...
                                <span className="text-gray-600">
                                    {challenger?.lastUpdated ?
                                        new Date(challenger.lastUpdated).toLocaleTimeString() : 'N/A'}
                                </span>
                            </div>
                            <div className="flex justify-between">
                            ...
                            <div className="flex justify-between">
                                ...
                                <span className="text-gray-600">
                                    {opponent?.lastUpdated ?
                                        new Date(opponent.lastUpdated).toLocaleTimeString() : 'N/A'}
                                </span>
                            </div>
                            <div className="flex justify-between">
                                <span>Milestones:</span>
            ...
            {showDetailedStats && (challenger?.milestones || opponent?.milestones) && (
                            ...
                            {challenger?.milestones && challenger.milestones.length > 1 && (
                                <svg className="absolute inset-0 w-full h-full">
                                    <polyline
                                        ...
                                        strokeWidth="2"
                                        points={challenger.milestones.map((value, index) => {
                                            const x = (index / (challenger.milestones.length - 1)) * 100;
                            ...
                            {opponent?.milestones && opponent.milestones.length > 1 && (
                                <svg className="absolute inset-0 w-full h-full">
                                    <polyline
                                        ...
                                        strokeWidth="2"
                                        points={opponent.milestones.map((value, index) => {
                                            const x = (index / (opponent.milestones.length - 1)) * 100;
                                            const y = 100 - ((value / maxProgress) * 100);
                                            return `${x}%,${y}%`;
                                        }).join(' ')}
                                    />
