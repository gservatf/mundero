...
import { Duel, DuelInvitation, DuelTemplate, DuelStatus, DUEL_TEMPLATES } from './types';
...
export const useDuels = () => {
    ...
    const loadDuels = useCallback(async (status?: DuelStatus) => {
        try {
            setLoading(true);
            setError(null);
    ...
    const createDuel = useCallback(async (
        ...
        try {
            const result = await duelService.createDuel(template, user.id, opponentId, customMessage);
    ...
    // Cancelar duelo
    const cancelDuel = useCallback(async (duelId: string): Promise<boolean> => {
        if (!user) {
            setError('User not authenticated');
            return false;
        }
    ...
    const joinAsSpectator = useCallback(async (duelId: string): Promise<boolean> => {
    ...
    const leaveAsSpectator = useCallback(async (duelId: string): Promise<boolean> => {
        if (!user) return false;
    ...
    const getDuelProgress = useCallback((duel: Duel, userId?: string) => {
        const targetUserId = userId || user?.id;
        if (!targetUserId || !duel.progress[targetUserId]) {
            return { current: 0, opponent: 0, isWinning: false, difference: 0 };
        }

        const opponentId = duel.challengerId === targetUserId ? duel.opponentId : duel.challengerId;
    ...
    const getTimeRemaining = useCallback((duel: Duel): {
        hours: number;
        minutes: number;
        isExpired: boolean;
        percentage: number;
    ...
    const isUserParticipating = useCallback((duel: Duel): boolean => {
        return user ? (duel.challengerId === user.id || duel.opponentId === user.id) : false;
    }, [user]);

    // Verificar si el usuario es espectador
    const isUserSpectator = useCallback((duel: Duel): boolean => {
        return user && duel.spectators ? duel.spectators.includes(user.id) : false;
    }, [user]);
