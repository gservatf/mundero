...
class DuelService {
  ...
  private simulateProgressUpdates() {
    ...
    activeDuels.forEach((duel) => {
      ...
      const now = Date.now();
      const endTime =
        (duel.startedAt || duel.createdAt) +
        duel.durationHours * 60 * 60 * 1000;
  ...
  async getUserInvitations(
    userId: string,
    status?: DuelInvitation["status"],
  ): Promise<DuelInvitation[]> {
    await new Promise((resolve) => setTimeout(resolve, 50));

    let filtered = this.invitations.filter(
      (inv) => inv.toUserId === userId || inv.fromUserId === userId,
    );
  ...
  async getUserDuelStats(userId: string): Promise<{
    ...
    const losses = finishedDuels.filter(
      (d) => d.winnerId && d.winnerId !== userId,
    ).length;
    const draws = finishedDuels.filter((d) => d.isDraw).length;
    const activeDuels = userDuels.filter((d) => d.status === "active").length;

    const winRate =
      finishedDuels.length > 0 ? (wins / finishedDuels.length) * 100 : 0;
