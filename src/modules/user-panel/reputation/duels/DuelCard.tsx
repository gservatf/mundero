...
import {
  Duel,
  METRIC_CONFIG,
  DUEL_DIFFICULTY_CONFIG,
  DUEL_CATEGORY_CONFIG,
} from "./types";
          ...
          <div className="flex items-center space-x-2">
            ...
            <span
              className={`px-2 py-1 text-xs rounded-full border ${
                DUEL_DIFFICULTY_CONFIG[duel.difficulty].color
              }`}
            >
              {DUEL_DIFFICULTY_CONFIG[duel.difficulty].icon}{" "}
              {DUEL_DIFFICULTY_CONFIG[duel.difficulty].name}
            </span>

            <span
              className={`px-2 py-1 text-xs rounded-full ${
                DUEL_CATEGORY_CONFIG[duel.category].color
              }`}
            >
        ...
        <div className="flex items-center justify-between text-sm text-gray-600">
          <div className="flex items-center space-x-4">
            <div className="flex items-center">
              <span
                className={`w-4 h-4 mr-2 ${metric.color.split(" ")[0]} rounded`}
              >
                {metric.icon}
              </span>
              {metric.name}
          ...
          {duel.status === "active" && (
            <div
              className={`flex items-center ${
                timeRemaining.hours < 2 ? "text-red-600" : "text-gray-600"
              }`}
            >
            ...
            {progress.difference > 0 && (
              <motion.div
                ...
                className={`text-center text-sm ${
                  progress.isWinning ? "text-green-600" : "text-red-600"
                }`}
              >
                {progress.isWinning ? "🚀 Vas ganando" : "💪 Detrás"} por{" "}
                {progress.difference}
              </motion.div>
            )}
          ...
          <div className="flex items-center justify-between text-sm">
            <div>
              ...
              <div className="font-medium">
                {duel.challengerId}:{" "}
                {duel.progress[duel.challengerId]?.current || 0} vs{" "}
                {duel.opponentId}:{" "}
                {duel.progress[duel.opponentId]?.current || 0}
              </div>
            </div>
        ...
        <div className="px-4 py-2 border-t border-gray-100 bg-gray-50">
          <div className="flex items-center justify-between text-sm text-gray-600">
            <div className="flex items-center">
              ...
              {duel.spectators.length} espectador
              {duel.spectators.length !== 1 ? "es" : ""}
            </div>
          </div>
        </div>
      )}
