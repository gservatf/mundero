...
export const UserStatsDetail: React.FC<UserStatsDetailProps> = ({
  ...
  const challengeWinRate =
    stats.challengesTotal > 0
      ? (stats.challengesWon / stats.challengesTotal) * 100
      : 0;
  const duelWinRate =
    stats.duelsTotal > 0 ? (stats.duelsWon / stats.duelsTotal) * 100 : 0;
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="flex items-center space-x-2">
                    <span
                      className={`text-2xl font-bold ${getTrendColor(rankChange)}`}
                    >
                      {rankChange > 0
                        ? `+${rankChange}`
                        : rankChange === 0
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              ...
              <div className="text-center p-4 bg-gray-50 rounded-lg">
                <p className="text-2xl font-bold text-orange-600">
                  {Math.round(stats.avgPointsPerDay)}
                </p>
                ...
                {stats.pointsThisWeek.map((points, index) => (
                  ...
                  >
                    <motion.div
                      initial={{ height: 0 }}
                      animate={{
                        height: `${(points / Math.max(...stats.pointsThisWeek)) * 100}%`,
                      }}
