...
interface UseOnboardingProgressReturn {
    ...
    updateStepProgress: (stepId: string | number, value: number) => Promise<boolean>;
...
export const useOnboardingProgress = (): UseOnboardingProgressReturn => {
    ...
    const loadProgress = useCallback(async () => {
        ...
        try {
            ...
            const userProgress = await onboardingService.getOnboardingProgress(user.id);
    ...
    const initializeOnboarding = useCallback(async (templateId?: string): Promise<boolean> => {
        ...
        try {
            ...
            const newProgress = await onboardingService.initializeOnboarding(user.id, templateId);
    ...
    // Completar paso
    const completeStep = useCallback(async (stepId: string | number): Promise<boolean> => {
        if (!user?.id) return false;

        try {
            const success = await onboardingService.completeStep(user.id, String(stepId));
            if (success) {
    ...
    // Saltar paso
    const skipStep = useCallback(async (stepId: string | number): Promise<boolean> => {
        if (!user?.id) return false;

        try {
            const success = await onboardingService.skipStep(user.id, String(stepId));
            if (success) {
                // El progreso se actualizará automáticamente por el listener
            } else {
    ...
    const updateStepProgress = useCallback(async (stepId: string | number, value: number): Promise<boolean> => {
        if (!user?.id) return false;
    ...
    const currentStep = progress?.steps.find(step => step.id === progress.currentStepId) || null;
...
interface UseQuestTemplatesReturn {
    ...
    createTemplate: (template: Omit<QuestTemplate, 'id' | 'createdAt' | 'updatedAt'>) => Promise<string>;
    updateTemplate: (templateId: string, updates: Partial<QuestTemplate>) => Promise<boolean>;
}
...
export const useQuestTemplates = (): UseQuestTemplatesReturn => {
    ...
    const createTemplate = useCallback(async (
        ...
        try {
            setError(null);
            const templateId = await onboardingService.createQuestTemplate(template);
