...
import { OnboardingProgress, OnboardingStep, QuestTemplate, OnboardingStats, OnboardingEvent } from './types';
...
class OnboardingService {
    ...
    private readonly DEFAULT_TEMPLATE: Omit<QuestTemplate, 'id' | 'createdAt' | 'updatedAt' | 'createdBy'> = {
        name: 'Bienvenido a Mundero',
        description: 'Completa estas misiones para comenzar tu aventura en la plataforma',
        isActive: true,
        category: 'standard',
    ...
    async initializeOnboarding(userId: string, templateId?: string): Promise<OnboardingProgress> {
        try {
            ...
            const template = templateId ? await this.getQuestTemplate(templateId) : null;
    ...
    async completeStep(userId: string, stepId: string): Promise<OnboardingProgress | null> {
        try {
            ...
            const completedSteps = progress.steps.filter(s => s.isCompleted || s.done).length;
            progress.completionPercentage = Math.round((completedSteps / progress.totalSteps) * 100);
            ...
            const newBadges = step.badge ? [...currentBadges, step.badge] : currentBadges;
            ...
            if (reputationService && typeof reputationService.logAction === 'function') {
                await reputationService.logAction(userId, 'onboarding_step_completed', {
                    stepId: stepId,
            ...
            if (progress.isCompleted) {
                await this.logOnboardingEvent(userId, 'quest_completed', {
                    ...
                    completionTimeMs: progress.completedAt ?
                        new Date(progress.completedAt).getTime() - new Date(progress.startedAt).getTime() : 0
                });
            }
    ...
    async skipStep(userId: string, stepId: string): Promise<OnboardingProgress | null> {
        try {
            const progress = await this.getOnboardingProgress(userId);
    ...
    listenToOnboardingProgress(userId: string, callback: (progress: OnboardingProgress | null) => void) {
        const docRef = doc(db, this.COLLECTION, userId);
    ...
    async createQuestTemplate(template: Omit<QuestTemplate, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
        try {
    ...
    async updateQuestTemplate(templateId: string, updates: Partial<QuestTemplate>): Promise<void> {
        try {
            const docRef = doc(db, this.TEMPLATES_COLLECTION, templateId);
    ...
    async getDetailedStats(): Promise<OnboardingStats> {
        try {
            ...
            const allProgress = progressSnapshot.docs.map(doc => doc.data() as OnboardingProgress);
            ...
            const averageProgress = totalUsers > 0
                ? allProgress.reduce((sum, p) => sum + (p.completionPercentage || 0), 0) / totalUsers
                : 0;

            const completionTimes = allProgress
                .filter(p => p.isCompleted && p.completedAt && p.startedAt)
                .map(p => {
                    const endTime = p.completedAt instanceof Date ? p.completedAt.getTime() : new Date(p.completedAt!).getTime();
                    const startTime = p.startedAt instanceof Date ? p.startedAt.getTime() : new Date(p.startedAt).getTime();
            ...
            const averagePoints = totalUsers > 0
                ? allProgress.reduce((sum, p) => sum + (p.totalPoints || 0), 0) / totalUsers
                : 0;
    ...
    private async logOnboardingEvent(userId: string, eventType: OnboardingEvent, data: any): Promise<void> {
        try {
            const event = {
                userId,
                eventType,
    ...
    getNextRecommendedStep(progress: OnboardingProgress): OnboardingStep | null {
        ...
        return progress.steps.find(step => !(step.isCompleted || step.done)) || null;
    }
    ...
    calculateCompletionPercentage(progress: OnboardingProgress): number {
        ...
        const completedSteps = progress.steps.filter(step => step.isCompleted || step.done).length;
