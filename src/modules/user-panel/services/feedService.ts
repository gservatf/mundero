...
export const feedService = {
     ...
     */
    async createPost(uid: string, postData: {
            ...
            if (REPUTATION_ENABLED) {
                try {
                    await reputationService.logAction(uid, 'post_create', {
                        ...
                        hasMedia: Boolean(postData.content.media?.length || postData.content.mediaUrls?.length)
                    });
                } catch (error) {
    ...
    async getGlobalFeed(options: {
        ...
        try {
            console.log('🌍 Loading global feed...', { limitCount: options.limitCount });
     ...
     */
    async getUserFeed(uid: string, options: {
        limitCount?: number;
        lastDoc?: QueryDocumentSnapshot;
    } = {}): Promise<{
        posts: FeedPost[];
        ...
        try {
            console.log('👤 Loading user feed...', { uid, limitCount: options.limitCount });
            ...
            let posts: FeedPost[] = snapshot.docs.map(doc => ({
                id: doc.id,
                ...doc.data()
            } as FeedPost));
     ...
     */
    async getPublicFeedByUsername(username: string, options: {
        limitCount?: number;
        lastDoc?: QueryDocumentSnapshot;
    } = {}): Promise<{
        posts: FeedPost[];
     ...
     */
    async getUserFeedPreferences(uid: string): Promise<UserFeedPreferences | null> {
        try {
     ...
     */
    applyFeedFilters(posts: FeedPost[], preferences: UserFeedPreferences): FeedPost[] {
        return posts.filter(post => {
            ...
            if (preferences.muteKeywords.some(keyword => postText.includes(keyword.toLowerCase()))) {
                return false;
            }
     ...
     */
    async togglePostLike(postId: string, uid: string, userData: { userName: string; userAvatar?: string }): Promise<{ liked: boolean; likesCount: number }> {
        try {
            ...
            if (isCurrentlyLiked) {
                ...
                const postDoc = await getDoc(postRef);
                const likesCount = postDoc.exists() ? (postDoc.data() as FeedPost).engagement.likes : 0;
     ...
     */
    async addComment(postId: string, uid: string, commentData: {
        content: string;
        authorName: string;
        authorAvatar?: string;
        ...
        try {
            ...
            console.log('✅ Comment added successfully', { commentId: commentDoc.id });
    ...
    async getPostComments(postId: string, options: {
        ...
        try {
            console.log('📄 Loading post comments...', { postId, limitCount: options.limitCount });
            ...
            const comments: FeedComment[] = snapshot.docs.map(doc => ({
                id: doc.id,
                ...doc.data()
            } as FeedComment));
     ...
     */
    listenToPostComments(postId: string, callback: (comments: FeedComment[]) => void): () => void {
        ...
        const unsubscribe = onSnapshot(commentsQuery, (snapshot) => {
            ...
            console.log('🔄 Comments updated via listener', { commentsCount: comments.length });
            callback(comments);
        }, (error) => {
     ...
     * Delete a comment
     */
    async deleteComment(postId: string, commentId: string, uid: string, isAdmin: boolean = false): Promise<void> {
        try {
            console.log('🗑️ Deleting comment...', { postId, commentId, uid, isAdmin });
     ...
     */
    async deletePostEnhanced(postId: string, uid: string, isAdmin: boolean = false): Promise<void> {
        try {
