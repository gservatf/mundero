...
export const agreementService = {
  ...
  async signUserAgreement(
    ...
    try {
      ...
      const agreementRef = doc(
        db,
        "user_agreements",
        `${uid}_${agreementType}_${Date.now()}`,
      );
      ...
      await setDoc(
        statusRef,
        {
          ...
          [statusField]: true,
          lastUpdated: serverTimestamp(),
          [`currentVersions.${this.getVersionFieldName(agreementType)}`]:
            CURRENT_AGREEMENT_VERSIONS[agreementType],
        },
        { merge: true },
      );
  ...
  async revokeUserAgreement(
    ...
    try {
      console.log("❌ Revoking user agreement...", {
        uid,
        agreementType,
        reason,
      });
      ...
      await setDoc(
        statusRef,
        {
          ...
          [`metadata.${agreementType}_revokedReason`]:
   ...
   */
  listenToAgreementStatus(
    uid: string,
    callback: (status: AgreementStatus | null) => void,
  ): () => void {
  ...
  async checkForUpdatedAgreements(uid: string): Promise<{
    ...
    try {
      ...
      if (
        !status.hasSignedTerms ||
        status.currentVersions.terms !==
          CURRENT_AGREEMENT_VERSIONS.terms_of_service
      ) {
        outdatedAgreements.push("terms_of_service");
      }
      if (
        !status.hasSignedPrivacy ||
        status.currentVersions.privacy !==
          CURRENT_AGREEMENT_VERSIONS.privacy_policy
      ) {
        outdatedAgreements.push("privacy_policy");
      }
      if (
        !status.hasSignedDataProcessing ||
        status.currentVersions.dataProcessing !==
          CURRENT_AGREEMENT_VERSIONS.data_processing
      ) {
