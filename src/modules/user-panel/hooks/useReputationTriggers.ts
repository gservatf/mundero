...
export function useReputationTriggers() {
  ...
  const isInCooldown = useCallback(
    (eventType: string, context?: string): boolean => {
  ...
  const updateCooldown = useCallback(
    (eventType: string, context?: string): void => {
      const key = `${eventType}_${context || "default"}`;
   ...
   */
  const triggerReputationEvent = useCallback(
    async (eventData: TriggerEventData): Promise<boolean> => {
      ...
      if (isInCooldown(eventType, contextKey)) {
        console.log(
          `Reputation event ${eventType} is in cooldown for context ${contextKey}`,
        );
        return false;
      }
      ...
      try {
        ...
        console.log(
          `Reputation event triggered: ${eventType} → ${reputationAction}`,
          metadata,
        );
        return true;
      } catch (error) {
  ...
  const setupMessageReactionListener = useCallback(() => {
    ...
    const handleMessageReaction = (reactionData: any) => {
      ...
      if (
        reactionData.messageAuthorId === user.id &&
        reactionData.type === "positive"
      ) {
        triggerReputationEvent({
  ...
  const setupLiveParticipationListener = useCallback(() => {
    ...
    const handleLiveParticipation = (participationData: any) => {
      if (
        participationData.userId === user.id &&
        participationData.duration > 300000
      ) {
        // 5 min+
        triggerReputationEvent({
  ...
  const manualTrigger = useCallback(
    (eventType: ReputationTriggerEvent, metadata?: Record<string, any>) => {
  ...
  const getCooldownStatus = useCallback(
    (eventType: string, context?: string) => {
      const key = `${eventType}_${context || "default"}`;
      const entry = cooldownRef.current.get(key);
      ...
      const remainingTime = Math.max(
        0,
        COOLDOWN_DURATION - timeSinceLastTrigger,
      );
