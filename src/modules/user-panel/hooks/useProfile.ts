...
export const useProfile = (): UseProfileReturn => {
    ...
    // Create profile
    const createProfile = useCallback(async (initialData: Partial<UserProfile>): Promise<void> => {
        if (!user?.id) {
            throw new Error('User not authenticated');
        }
        ...
        try {
            ...
            console.log('🆕 Creating profile via hook...', { uid: user.id, initialData });
    ...
    useEffect(() => {
        ...
        // Setup real-time listener
        const unsubscribe = profileService.listenToUserProfile(user.id, (profile) => {
            console.log('🔄 Profile updated via listener:', profile ? 'Profile found' : 'Profile not found');
            setProfile(profile);
            setProfileExists(!!profile);
            setLoading(false);
        });
        ...
        profileService.profileExists(user.id).then(exists => {
