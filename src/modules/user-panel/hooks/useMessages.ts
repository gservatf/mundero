...
export const useMessages = () => {
  ...
  // Real-time listeners
  const [chatsUnsubscribe, setChatsUnsubscribe] = useState<(() => void) | null>(
    null,
  );
  const [messagesUnsubscribe, setMessagesUnsubscribe] = useState<
    Record<string, () => void>
  >({});
  ...
  const setLoadingState = useCallback(
    (
      key: keyof Pick<
        MessagesState,
        "isLoading" | "isLoadingChats" | "isLoadingMessages" | "isSending"
      >,
      loading: boolean,
    ) => {
  ...
  return {
    ...
    currentMessages: state.currentChatId
