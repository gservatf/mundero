...
export const useAgreement = (): UseAgreementReturn => {
  ...
  const hasSignedAgreement = useCallback(
    (type: UserAgreement["agreementType"]): boolean => {
  ...
  // Revoke an agreement
  const revokeAgreement = useCallback(
    async (
      type: UserAgreement["agreementType"],
      reason?: string,
    ): Promise<void> => {
      ...
      try {
        ...
        console.log("❌ Revoking agreement via hook...", {
          uid: user.id,
          type,
          reason,
        });
        ...
        const errorMessage =
          error instanceof Error ? error.message : "Failed to revoke agreement";
  ...
  const checkForUpdates = useCallback(async (): Promise<{
    ...
    try {
      console.log("🔍 Checking for agreement updates via hook...", {
        uid: user.id,
      });
      const updateInfo = await agreementService.checkForUpdatedAgreements(
        user.id,
      );
  ...
  const refreshAgreements = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Failed to refresh agreements";
  ...
  useEffect(() => {
    ...
    console.log("👂 Setting up agreement listener via hook...", {
      uid: user.id,
    });
    setLoading(true);
    setError(null);
    ...
    // Setup real-time listener
    const unsubscribe = agreementService.listenToAgreementStatus(
      user.id,
      (status) => {
        console.log(
          "🔄 Agreement status updated via listener:",
          status ? "Status found" : "Status not found",
        );
        setAgreementStatus(status);
        setLoading(false);
      },
