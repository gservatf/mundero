...
import { Crown, Shield, UserCheck, User, MoreHorizontal, Users } from 'lucide-react';
...
export const MemberList: React.FC<MemberListProps> = ({
    ...
    const sortedMembers = [...members].sort((a, b) => {
        ...
        const aDate = a.joinedAt && typeof a.joinedAt.toDate === 'function'
            ? a.joinedAt.toDate()
            : a.joinedAt instanceof Date
                ? a.joinedAt
                : new Date();
        const bDate = b.joinedAt && typeof b.joinedAt.toDate === 'function'
            ? b.joinedAt.toDate()
        ...
        <div className="space-y-4">
                    ...
                    >
                        ...
                        {canManageMember(member.role, member.userId) && (
                            <div className="flex items-center gap-2">
                                ...
                                {member.role === 'moderator' && onPromote && currentUserRole === 'owner' && (
                                    <Button
                                        variant="outline"
                                        size="sm"
                        ...
                        <div className="hidden md:flex items-center gap-1 text-xs text-gray-400">
                            {member.permissions.canPost && <span title="Puede publicar">📝</span>}
                            {member.permissions.canComment && <span title="Puede comentar">💬</span>}
                            {member.permissions.canInvite && <span title="Puede invitar">👥</span>}
                            {member.permissions.canModerate && <span title="Puede moderar">🛡️</span>}
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div className="mt-6 p-3 bg-blue-50 rounded-lg border border-blue-200">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs text-blue-700">
                    <div><strong>Propietario:</strong> Control total</div>
                    <div><strong>Administrador:</strong> Gestión de miembros y contenido</div>
                    <div><strong>Moderador:</strong> Moderar contenido</div>
                    <div><strong>Miembro:</strong> Participar en la comunidad</div>
                </div>
            </div>
        </div>
    );
