...
class CommunityService {
    ...
    async createCommunity(userId: string, userName: string, data: CreateCommunityData): Promise<string> {
        try {
            ...
            const docRef = await addDoc(collection(db, this.communitiesCollection), communityData);
    ...
    async getCommunities(userId?: string, filters?: {
        category?: string;
        search?: string;
        location?: string;
        limit?: number;
        ...
        try {
            ...
            if (userId) {
                ...
                communities = communities.filter(c =>
                    !c.isPrivate || userCommunityIds.includes(c.id)
                );
            } else {
    ...
    // Join a community
    async joinCommunity(communityId: string, userId: string, userName: string): Promise<void> {
        try {
            ...
            if (status === 'active') {
                await this.updateCommunityStats(communityId, { memberCount: increment(1) });
            }
    ...
    // Get community posts
    async getCommunityPosts(communityId: string, lastDoc?: any, limitCount: number = 20): Promise<{
        posts: CommunityPost[];
        lastDoc: any;
        hasMore: boolean;
    }> {
    ...
    async createCommunityPost(
        ...
        try {
            ...
            if (!member.permissions.canPost && member.role !== 'owner' && member.role !== 'admin') {
                throw new Error('No permission to post in this community');
            }

            const postData: Omit<FeedPost, 'id'> & { communityId: string; communityName: string; isPinned?: boolean; isAnnouncement?: boolean } = {
                authorId,
                authorName,
            ...
            const docRef = await addDoc(collection(db, this.postsCollection), postData);
    ...
    async getCommunityMembers(communityId: string, role?: string): Promise<CommunityMember[]> {
        try {
            let q = query(
    ...
    private async getMember(communityId: string, userId: string): Promise<CommunityMember | null> {
        try {
            const q = query(
                collection(db, this.membersCollection),
                where('communityId', '==', communityId),
    ...
    private async updateCommunityStats(communityId: string, updates: any): Promise<void> {
        try {
            const docRef = doc(db, this.communitiesCollection, communityId);
    ...
    // Real-time listeners
    onCommunityUpdates(communityId: string, callback: (community: Community) => void): () => void {
        const docRef = doc(db, this.communitiesCollection, communityId);
    ...
    onCommunityMembers(communityId: string, callback: (members: CommunityMember[]) => void): () => void {
        const q = query(
            collection(db, this.membersCollection),
            where('communityId', '==', communityId),
            orderBy('joinedAt', 'desc')
    ...
    async updateCommunity(communityId: string, uid: string, data: Partial<CreateCommunityData>): Promise<void> {
        try {
    ...
    async promoteToAdmin(communityId: string, targetUserId: string, promoterUid: string): Promise<void> {
        try {
    ...
    async removeAdmin(communityId: string, targetUserId: string, removerUid: string): Promise<void> {
        try {
            const community = await this.getCommunity(communityId);
            if (!community) {
                throw new Error('Comunidad no encontrada');
    ...
    async kickMember(communityId: string, targetUserId: string, kickerUid: string): Promise<void> {
        try {
            const community = await this.getCommunity(communityId);
            if (!community) {
                throw new Error('Comunidad no encontrada');
            ...
            if (community.admins.includes(targetUserId) && community.createdBy !== kickerUid) {
