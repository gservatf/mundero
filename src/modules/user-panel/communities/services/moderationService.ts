...
class ModerationService {
  ...
  async assignReport(
    ...
    try {
      ...
      const canAssign = await this.canUserModerate(
        report.communityId,
        assignedBy,
      );
      if (!canAssign) {
  ...
  async resolveReport(
    ...
    try {
      ...
      const canModerate = await this.canUserModerate(
        report.communityId,
        moderatorId,
      );
      if (!canModerate) {
  ...
  private async banUser(
    ...
    const banUntil =
      !permanent && duration ? new Date(Date.now() + duration * 1000) : null;
  ...
  getReportReasonInfo(reason: ReportReason): {
    name: string;
    color: string;
    icon: string;
  } {
