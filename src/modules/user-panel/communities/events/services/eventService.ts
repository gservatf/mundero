...
class EventService {
    ...
    // Crear evento
    async createEvent(communityId: string, data: CreateEventData & { createdBy: string; createdByName?: string }): Promise<string> {
        try {
            const eventData: Omit<CommunityEvent, 'id'> = {
                ...
                embedUrl: data.embedUrl ? this.validateEmbedUrl(data.embedUrl) : undefined,
    ...
    async getEventsByCommunity(communityId: string, limitCount: number = 20): Promise<CommunityEvent[]> {
        try {
            const q = query(
    ...
    listenToEvents(communityId: string, callback: (events: CommunityEvent[]) => void): () => void {
        const q = query(
            collection(db, this.eventsCollection),
            where('communityId', '==', communityId),
            orderBy('startAt', 'asc')
    ...
    // Actualizar evento
    async updateEvent(communityId: string, eventId: string, updateData: Partial<CommunityEvent>): Promise<void> {
        try {
    ...
    // Unirse a evento
    async joinEvent(communityId: string, eventId: string, userId: string): Promise<void> {
        try {
    ...
    // Salir de evento
    async leaveEvent(communityId: string, eventId: string, userId: string): Promise<void> {
        try {
            await updateDoc(doc(db, this.eventsCollection, eventId), {
    ...
    async updateEventStatus(eventId: string, status: CommunityEvent['status'], isLive?: boolean): Promise<void> {
        try {
    ...
    filterEventsByStatus(events: CommunityEvent[], status: CommunityEvent['status'][]): CommunityEvent[] {
        return events.filter(event => status.includes(event.status));
    }

    // Filtrar eventos por fecha
    filterEventsByDate(events: CommunityEvent[], startDate?: number, endDate?: number): CommunityEvent[] {
