...
export function EventList({
  ...
  const [eventsListener, setEventsListener] = useState<(() => void) | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const unsubscribe = eventService.listenToEvents(
      communityId,
      (eventsData) => {
  ...
  useEffect(() => {
    ...
    if (statusFilter !== "all") {
      switch (statusFilter) {
        case "upcoming":
          filtered = filtered.filter(
            (event) =>
              event.startAt > Date.now() && event.status === "scheduled",
          );
          break;
        case "live":
          filtered = filtered.filter((event) =>
            eventService.isEventLive(event),
          );
          break;
        case "past":
          filtered = filtered.filter(
            (event) => event.startAt < Date.now() && event.status === "ended",
          );
          break;
        case "my-events":
          filtered = filtered.filter(
    ...
    if (timeFilter !== "all") {
      ...
      switch (timeFilter) {
        ...
        case "this-week":
          filtered = filtered.filter(
            (event) => event.startAt >= now && event.startAt <= now + oneWeek,
          );
          break;
        case "this-month":
          filtered = filtered.filter(
            (event) => event.startAt >= now && event.startAt <= now + oneMonth,
          );
          break;
      }
    }
  ...
  const getFilterStats = () => {
    ...
    const upcoming = events.filter(
      (e) => e.startAt > now && e.status === "scheduled",
    ).length;
    const live = events.filter((e) => eventService.isEventLive(e)).length;
    const past = events.filter(
      (e) => e.startAt < now && e.status === "ended",
    ).length;
    const myEvents = currentUserId
      ? events.filter((e) => e.attendees?.includes(currentUserId)).length
      : 0;
