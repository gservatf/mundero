...
export function EventReminderButton({
    ...
    const checkExistingReminder = () => {
        ...
        const hasReminder = reminders.some(r => r.eventId === event.id && r.userId === userId);
    ...
    const scheduleReminder = (reminderTime: number) => {
        ...
        if (timeUntilReminder <= 0) {
            console.log('El evento ya ha comenzado o el recordatorio es para el pasado');
            return;
        }
    ...
    const addReminder = () => {
        ...
        const updatedReminders = reminders.filter(r => !(r.eventId === event.id && r.userId === userId));
    ...
    return (
        <Button
            ...
            size="sm"
            className={`transition-all duration-200 ${reminderEnabled
        ...
        >
            ...
            {loading ? 'Guardando...' : reminderEnabled ? 'Recordatorio activo' : 'Activar recordatorio'}
...
export function useEventReminders() {
    ...
    const scheduleReminderNotification = (reminder: EventReminder, timeUntil: number) => {
        setTimeout(() => {
            if ('Notification' in window && Notification.permission === 'granted') {
                new Notification(`¡Recordatorio de evento!`, {
    ...
    const clearExpiredReminders = () => {
        const stored = getStoredReminders();
        const now = Date.now();
        const validReminders = stored.filter((reminder: EventReminder) =>
            reminder.eventStartTime > now
        );
