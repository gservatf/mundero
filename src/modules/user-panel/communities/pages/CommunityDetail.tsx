...
import { communityService, Community, CommunityMember, CommunityPost } from '../services/communityService';
...
export const CommunityDetail: React.FC = () => {
    ...
    const [membersListener, setMembersListener] = useState<(() => void) | null>(null); // FASE 6.1: Listener tiempo real
    ...
    const loadCommunityData = async () => {
        ...
        try {
            ...
            if (user?.id) {
                ...
                if (adminStatus) {
                    const unsubscribe = communityService.onCommunityMembers(communityId, (updatedMembers) => {
                        setMembers(updatedMembers);
                        // Actualizar userMembership si cambió
                        const updatedUserMembership = updatedMembers.find(m => m.userId === user.id);
            ...
            const canViewPosts = !communityData.isPrivate ||
                (user?.id && membersData.some(m => m.userId === user.id && m.status === 'active'));
    ...
    const loadPosts = async () => {
        ...
        try {
            ...
            const { posts: postsData } = await communityService.getCommunityPosts(communityId);
            setPosts(postsData);
        } catch (error) {
    ...
    const handlePromoteMember = async (memberId: string, newRole: string) => {
        ...
        try {
            ...
            if (newRole === 'admin') {
                await communityService.promoteToAdmin(communityId, member.userId, user.id);
    ...
    const handleKickMember = async (memberId: string) => {
        ...
        if (confirm(`¿Estás seguro de que quieres expulsar a ${member.userName} de la comunidad?`)) {
            try {
    ...
    const canPost = () => {
        return userMembership &&
            ...
            (userMembership.permissions.canPost || ['owner', 'admin'].includes(userMembership.role));
    };
    ...
    if (!community) {
        return (
            <div className="text-center py-12">
                <h2 className="text-xl font-semibold text-gray-600">Comunidad no encontrada</h2>
                ...
                <div className="p-6">
                        ...
                        <div className="flex flex-col items-end gap-4">
                            {user && (
                                <JoinButton
                                    ...
                                    isJoined={!!userMembership && userMembership.status === 'active'}
                    ...
                    {community.rules && community.rules.length > 0 && (
                        <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
                            <h3 className="font-semibold text-blue-900 mb-2">Reglas de la Comunidad</h3>
                    ...
                    <Card className="p-6">
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <div className="space-y-2 text-sm text-gray-600">
                                        ...
                                        {community.location && (
                                            <div>
                                                Ubicación: {community.location.city || community.location.country}
                                            </div>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="space-y-2 text-sm text-gray-600">
                                        ...
                                        <div>Total de comentarios: {community.stats.totalComments}</div>
                            ...
                            <div>
                                ...
                                <div className="space-y-2 text-sm text-gray-600">
                                    <div>
                                        Invitaciones: {community.settings.allowInvites ? 'Permitidas' : 'No permitidas'}
                                    </div>
                                    <div>
                                        Aprobación requerida: {community.settings.requireApproval ? 'Sí' : 'No'}
                                    </div>
                                    <div>
                                        Publicaciones de miembros: {community.settings.allowMemberPosts ? 'Permitidas' : 'Solo administradores'}
                                    </div>
                                    <div>
                                        Eventos de miembros: {community.settings.allowMemberEvents ? 'Permitidos' : 'Solo administradores'}
                                    </div>
                                </div>
                            </div>
                        </div>
