...
class ApiClient {
  ...
  constructor() {
    ...
    this.baseUrl = (globalThis as any)?.process?.env?.VITE_MUNDERO_BASE_URL || 
                   (window as any)?.__ENV__?.VITE_MUNDERO_BASE_URL || 
                   'https://mundero.net';
    
    this.apiKey = (globalThis as any)?.process?.env?.MUNDERO_API_KEY || 
                  (window as any)?.__ENV__?.MUNDERO_API_KEY || 
                  '';
    
    this.version = 'v1';
    
    this.mode = (globalThis as any)?.process?.env?.VITE_API_MODE || 
  ...
  async del<T = any>(endpoint: string, config?: RequestConfig): Promise<ApiResponse<T>> {
  ...
  async healthCheck(): Promise<boolean> {
    try {
      const response = await this.get('health', { skipAuth: true, maxRetries: 1 });
