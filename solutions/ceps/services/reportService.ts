 ...
 */
export function validatePdfParams(params: Partial<PdfReportParams>): params is PdfReportParams {
    return !!(
        params.user?.name &&
        params.user?.email &&
        params.scores &&
 ...
 */
export function generatePdfFileName(userName: string, mode: "user" | "corporate"): string {
