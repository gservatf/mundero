 ...
 */
export async function pushReportToDriveAndSheet(payload: CorpPayload): Promise<DriveIntegrationResult> {
 ...
 */
export function validateCorpPayload(payload: Partial<CorpPayload>): payload is CorpPayload {
    return !!(
        payload.user?.name &&
        payload.user?.email &&
        payload.org?.id &&
...
export function generateCorpMetadata(payload: CorpPayload) {
    ...
    const scoreAverage = Object.values(payload.scores).reduce((sum, score) => sum + score, 0) / Object.values(payload.scores).length;
