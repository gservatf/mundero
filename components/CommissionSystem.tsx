...
import { 
  FiDollarSign, FiTrendingUp, FiCalendar, FiUser, FiCheck, FiClock, 
  FiX, FiDownload, FiEye, FiFilter, FiSearch, FiBarChart, FiPieChart
} from 'react-icons/fi';
...
const CommissionSystem = () => {
  ...
  const [commissions] = useState<Commission[]>([
    {
      ...
      description: 'Implementación completa de sistema legal para gestión de contratos',
    ...
    },
    {
      ...
      status: 'pending',
      date: '2024-01-28',
      description: 'Implementación de sistema de analytics y business intelligence',
      contractValue: 5000.00
    },
    {
      id: '5',
      referral: {
        ...
        avatar: 'https://ui-avatars.com/api/?name=Diego+Morales&background=ef4444&color=fff'
      },
      service: {
  ...
  const filteredCommissions = commissions.filter(commission => {
    const matchesFilter = activeFilter === 'all' || commission.status === activeFilter;
    const matchesSearch = commission.referral.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         commission.service.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         commission.referral.company.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-2xl font-bold text-gray-900">Sistema de Comisiones</h3>
          <p className="text-gray-600">Gestiona y monitorea tus comisiones por referidos</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-6 rounded-2xl border border-green-100 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-green-900">${stats.totalEarned.toLocaleString()}</p>
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-2xl border border-blue-100 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-blue-600 font-medium text-sm">Pendiente de Pago</p>
              <p className="text-3xl font-bold text-blue-900">${stats.pendingAmount.toLocaleString()}</p>
        ...
        <div className="bg-gradient-to-r from-purple-50 to-violet-50 p-6 rounded-2xl border border-purple-100 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-purple-900">${stats.thisMonth.toLocaleString()}</p>
        ...
        <div className="bg-gradient-to-r from-orange-50 to-amber-50 p-6 rounded-2xl border border-orange-100 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-orange-600 font-medium text-sm">Tasa Conversión</p>
              <p className="text-3xl font-bold text-orange-900">{stats.conversionRate}%</p>
              <p className="text-orange-600 text-sm">{stats.totalReferrals} referidos totales</p>
            </div>
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-start space-x-4 flex-1">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 mb-2">
                      <h4 className="font-bold text-gray-900 text-lg">{commission.referral.name}</h4>
                      <span className="text-gray-400">•</span>
                      <span className="text-gray-600">{commission.referral.company}</span>
                    </div>
                    
                    <div className="flex items-center space-x-3 mb-3">
                      <div className={`w-4 h-4 rounded-full ${commission.service.color}`}></div>
                      <span className="font-medium text-gray-900">{commission.service.name}</span>
                      <span className="px-2 py-1 bg-gray-100 text-gray-600 text-xs rounded-full">
                        {commission.percentage}% comisión
                      </span>
                    </div>
                    
                    <p className="text-gray-700 mb-4 leading-relaxed">{commission.description}</p>
                    
                    <div className="flex items-center space-x-6 text-sm text-gray-500">
                      <div className="flex items-center space-x-1">
                        <FiCalendar className="w-4 h-4" />
                        <span>Referido: {new Date(commission.date).toLocaleDateString()}</span>
                      </div>
                      {commission.paymentDate && (
                        <div className="flex items-center space-x-1">
                          <FiCheck className="w-4 h-4 text-green-500" />
                          <span>Pagado: {new Date(commission.paymentDate).toLocaleDateString()}</span>
                        </div>
                      )}
                      <div className="flex items-center space-x-1">
                        <FiDollarSign className="w-4 h-4" />
                        <span>Contrato: ${commission.contractValue.toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-4">
                  <div className="text-right">
                    ...
                    <div className={`inline-flex items-center space-x-1 px-3 py-1 rounded-full text-sm font-medium border ${getStatusColor(commission.status)}`}>
      ...
      {filteredCommissions.length === 0 && (
        <div className="bg-white rounded-2xl shadow-sm border p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-900 mb-2">No se encontraron comisiones</h3>
          <p className="text-gray-600">
            {searchTerm ? 'Intenta con otros términos de búsqueda' : 'Aún no tienes comisiones registradas'}
          </p>
        </div>
      )}
    </div>
