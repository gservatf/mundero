...
import { 
  FiHome, FiCheck, FiX, FiEye, FiEdit, FiTrash2, FiSearch, 
  FiFilter, FiDownload, FiUserCheck, FiAlertCircle, FiMapPin,
  FiPhone, FiMail, FiCalendar, FiUsers, FiLink, FiSave, FiFileText
} from 'react-icons/fi';
...
const CompanyManagement = () => {
  ...
  const [companies, setCompanies] = useState<Company[]>([
    {
      ...
      descripcion: 'Empresa especializada en construcción de obras civiles y edificaciones comerciales.',
      empresaInteres: 'We Consulting',
      notasContexto: 'Buscamos expandir nuestros servicios de consultoría en gestión de proyectos de construcción. Tenemos varios proyectos grandes en pipeline y necesitamos optimizar nuestros procesos.'
    },
    {
      id: '2',
      ruc: '20987654321',
      ...
      descripcion: 'Desarrollo de software y soluciones tecnológicas innovadoras.',
      empresaInteres: 'Legalty',
      notasContexto: 'Estamos interesados en integrar servicios legales automatizados en nuestras soluciones de software. Queremos ofrecer a nuestros clientes herramientas legales digitales.'
    },
    {
      id: '3',
      ruc: '20456789123',
      nombre: 'Consulting Pro EIRL',
      razonSocial: 'Consulting Pro Empresa Individual de Responsabilidad Limitada',
      sector: 'Consultoría',
      ...
      rejectionReason: 'Documentación incompleta. Falta vigencia de poderes actualizada.',
      descripcion: 'Servicios de consultoría empresarial y asesoría estratégica.',
      empresaInteres: 'Portales',
      notasContexto: 'Queremos desarrollar una plataforma web para ofrecer nuestros servicios de consultoría de manera digital y llegar a más clientes.'
    }
  ]);
    ...
    setCompanies(prev => prev.map(company => 
      company.id === companyId 
        ? { 
            ...company, 
            status: 'rechazada',
            rejectionReason: reason || 'Documentación no cumple con los requisitos'
          }
        : company
    ));
  ...
  const filteredCompanies = companies.filter(company => {
    const matchesSearch = company.nombre.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         ...
                         company.representanteLegal.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-2xl font-bold text-gray-900">Gestión de Empresas y Roles</h3>
          <p className="text-gray-600">Administra empresas registradas, documentación y solicitudes de roles</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-2xl border border-blue-100">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-blue-600 font-medium text-sm">Empresas Registradas</p>
              <p className="text-3xl font-bold text-blue-900">{companies.length}</p>
              <p className="text-blue-600 text-sm">{companies.filter(c => c.status === 'validada').length} validadas</p>
            </div>
        ...
        <div className="bg-gradient-to-r from-yellow-50 to-orange-50 p-6 rounded-2xl border border-yellow-100">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-yellow-900">{companies.filter(c => c.status === 'en_proceso').length}</p>
        ...
        <div className="bg-gradient-to-r from-purple-50 to-violet-50 p-6 rounded-2xl border border-purple-100">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-purple-600 font-medium text-sm">Solicitudes de Rol</p>
              <p className="text-3xl font-bold text-purple-900">{companyRoles.filter(r => r.status === 'pending').length}</p>
        ...
        <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-6 rounded-2xl border border-green-100">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-green-600 font-medium text-sm">Total Empleados</p>
              <p className="text-3xl font-bold text-green-900">{companies.reduce((sum, c) => sum + c.empleados, 0)}</p>
      ...
      <div className="bg-white rounded-2xl shadow-sm border">
        <div className="border-b border-gray-200">
          <nav className="flex space-x-8 px-6">
            ...
            >
              Solicitudes de Rol ({companyRoles.filter(r => r.status === 'pending').length})
            </button>
          </nav>
        </div>
        ...
        <div className="p-6 border-b border-gray-200">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder={activeTab === 'companies' ? "Buscar empresas, RUC, representante, empresa de interés..." : "Buscar usuarios, email..."}
                value={searchTerm}
        ...
        <div className="p-6">
                    ...
                    <div className="flex items-start space-x-4 flex-1">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center space-x-3 mb-2">
                          <h4 className="font-bold text-gray-900 text-lg">{company.nombre}</h4>
                          <span className="px-3 py-1 bg-blue-100 text-blue-700 text-xs rounded-full font-medium">
                            RUC: {company.ruc}
                          </span>
                          <div className={`inline-flex items-center space-x-1 px-3 py-1 rounded-full text-sm font-medium border ${getStatusColor(company.status)}`}>
                            {getStatusIcon(company.status)}
                            <span>{getStatusLabel(company.status)}</span>
                          </div>
                        </div>
                        
                        <p className="text-gray-700 mb-3">{company.razonSocial}</p>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-sm text-gray-600 mb-4">
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <span>Registrado: {new Date(company.fechaRegistro).toLocaleDateString()}</span>
                          </div>
                        </div>

                        {company.descripcion && (
                          <p className="text-gray-700 mb-3 italic">"{company.descripcion}"</p>
                        )}
                          ...
                          <div className="mb-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                            ...
                            {company.empresaInteres && (
                              <p className="text-blue-800 text-sm mb-2">
                                <strong>Empresa de interés:</strong> {company.empresaInteres}
                              </p>
                            )}
                          ...
                          {editingCompany?.id === company.id ? (
                            ...
                            <div>
                              {company.driveUrl ? (
                                ...
                                <span className="text-gray-500 text-sm">No se ha configurado URL de Drive</span>
                              )}
                            </div>
                          )}
                        </div>

                        {company.status === 'validada' && company.validatedBy && (
                          <div className="p-3 bg-green-50 rounded-lg border border-green-200">
                            <p className="text-green-800 text-sm">
                              <strong>Validada por:</strong> {company.validatedBy} el {new Date(company.validatedDate!).toLocaleDateString()}
                            </p>
                          </div>
                        )}

                        {company.status === 'rechazada' && company.rejectionReason && (
                          <div className="p-3 bg-red-50 rounded-lg border border-red-200">
                            <p className="text-red-800 text-sm">
                              <strong>Motivo de rechazo:</strong> {company.rejectionReason}
                            </p>
                          </div>
                        )}
                      </div>
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex items-start space-x-4 flex-1">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center space-x-3 mb-2">
                            <h4 className="font-bold text-gray-900 text-lg">{role.userName}</h4>
                            <span className="text-gray-400">•</span>
                            <span className="text-gray-600">{role.userEmail}</span>
                          </div>
                          ...
                          <div className="flex items-center space-x-6 text-sm text-gray-600">
                            <div>
                              <strong>Rol actual:</strong> {getRoleLabel(role.role)}
                            </div>
                            <div>
                              <strong>Rol solicitado:</strong> {getRoleLabel(role.requestedRole)}
                            </div>
                            <div>
                              <strong>Fecha solicitud:</strong> {new Date(role.requestDate).toLocaleDateString()}
                            </div>
                          </div>

                          {role.status === 'approved' && role.approvedBy && (
                            <div className="mt-2 text-sm text-green-600">
                              Aprobado por {role.approvedBy} el {new Date(role.approvedDate!).toLocaleDateString()}
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center space-x-2">
                        <div className={`inline-flex items-center space-x-1 px-3 py-1 rounded-full text-sm font-medium border ${getStatusColor(role.status)}`}>
          ...
          {activeTab === 'companies' && filteredCompanies.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">
                {searchTerm ? 'Intenta con otros términos de búsqueda' : 'Aún no hay empresas registradas'}
              </p>
            </div>
          )}
          ...
          {activeTab === 'roles' && filteredRoles.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">
                {searchTerm ? 'Intenta con otros términos de búsqueda' : 'No hay solicitudes de rol pendientes'}
              </p>
            </div>
          )}
        </div>
