...
import { 
  FiGrid, FiPlus, FiKey, FiEye, FiEyeOff, FiCopy, FiCheck, FiX, 
  FiSettings, FiTrash2, FiPause, FiPlay, FiLink, FiShield,
  FiActivity, FiCalendar, FiGlobe, FiLock, FiUnlock
} from 'react-icons/fi';
...
const ApplicationManagement = () => {
  const [apps, setApps] = useState<IntegratedApp[]>([
    {
      ...
      logoUrl: 'https://ui-avatars.com/api/?name=Legalty&background=3b82f6&color=fff',
    ...
    },
    {
      ...
      logoUrl: 'https://ui-avatars.com/api/?name=We+Consulting&background=10b981&color=fff',
    ...
    },
    {
      ...
      logoUrl: 'https://ui-avatars.com/api/?name=Portales&background=8b5cf6&color=fff',
  ...
  const [newAppCredentials, setNewAppCredentials] = useState<{apiKey: string, apiSecret: string} | null>(null);
  ...
  const availablePermissions = [
    { id: 'user.read', label: 'Leer usuarios', description: 'Acceso de lectura a datos de usuarios' },
    { id: 'user.write', label: 'Escribir usuarios', description: 'Crear y modificar usuarios' },
    { id: 'company.read', label: 'Leer empresas', description: 'Acceso a datos de empresas' },
    { id: 'company.sync', label: 'Sincronizar empresas', description: 'Sincronización bidireccional de empresas' },
    { id: 'lead.converted', label: 'Leads convertidos', description: 'Notificar conversiones de leads' },
    { id: 'commission.sync', label: 'Sincronizar comisiones', description: 'Gestión de comisiones y pagos' },
    { id: 'webhook.send', label: 'Enviar webhooks', description: 'Capacidad de enviar notificaciones' },
    { id: 'analytics.read', label: 'Leer analytics', description: 'Acceso a métricas y estadísticas' }
  ];
  ...
  const handleCreateApp = () => {
    if (!formData.nombre || !formData.urlBase || formData.permisos.length === 0) {
  ...
  const handleChangeStatus = (appId: string, newStatus: 'activo' | 'pausado' | 'revocado') => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-2xl font-bold text-gray-900">Gestión de Aplicaciones</h3>
          <p className="text-gray-600">Administra aplicaciones integradas, API Keys y permisos</p>
        </div>
        <button
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-2xl border border-blue-100">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-blue-600 text-sm">{apps.filter(a => a.estado === 'activo').length} activas</p>
            </div>
        ...
        <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-6 rounded-2xl border border-green-100">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-green-900">{apps.reduce((sum, app) => sum + app.requestsCount, 0).toLocaleString()}</p>
        ...
        <div className="bg-gradient-to-r from-purple-50 to-violet-50 p-6 rounded-2xl border border-purple-100">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-purple-900">{apps.reduce((sum, app) => sum + app.webhooksConfigured, 0)}</p>
        ...
        <div className="bg-gradient-to-r from-orange-50 to-red-50 p-6 rounded-2xl border border-orange-100">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-orange-900">{apps.filter(a => a.estado === 'pausado').length}</p>
                ...
                <div className="flex items-start space-x-4 flex-1">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 mb-2">
                      <h4 className="font-bold text-gray-900 text-lg">{app.nombre}</h4>
                      <span className="px-3 py-1 bg-blue-100 text-blue-700 text-xs rounded-full font-medium">
                        {app.slug}
                      </span>
                      <div className={`inline-flex items-center space-x-1 px-3 py-1 rounded-full text-sm font-medium border ${getStatusColor(app.estado)}`}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-sm text-gray-600 mb-4">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <span>{app.requestsCount.toLocaleString()} requests</span>
                      </div>
                      <div className="flex items-center space-x-2">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <span>Creado: {new Date(app.fechaCreacion).toLocaleDateString()}</span>
                      </div>
                      {app.ultimaSincronizacion && (
                        <div className="flex items-center space-x-2">
                          <FiCheck className="w-4 h-4 text-green-500" />
                          <span>Última sync: {new Date(app.ultimaSincronizacion).toLocaleDateString()}</span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="mb-4 p-4 bg-white rounded-lg border">
                      ...
                      <div className="space-y-3">
                        ...
                        <div>
                          ...
                          <div className="flex items-center space-x-2">
                            <code className="flex-1 px-3 py-2 bg-gray-100 rounded font-mono text-sm">
                              {showApiSecret === app.id ? app.apiSecret : '••••••••••••••••••••••••••••••••'}
                            </code>
                            <button
                              onClick={() => setShowApiSecret(showApiSecret === app.id ? null : app.id)}
                              className="p-2 text-gray-500 hover:text-gray-700"
                            >
                              {showApiSecret === app.id ? <FiEyeOff className="w-4 h-4" /> : <FiEye className="w-4 h-4" />}
                            </button>
                            <button
                              onClick={() => copyToClipboard(app.apiSecret, 'API Secret')}
                              className="p-2 text-gray-500 hover:text-gray-700"
                            >
                              <FiCopy className="w-4 h-4" />
                            </button>
        ...
        >
          ...
          >
            <div className="bg-gradient-to-r from-blue-600 to-purple-600 p-6 text-white">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-bold">Registrar Nueva Aplicación</h2>
                  <p className="text-blue-100">Genera credenciales API automáticamente</p>
                </div>
                <button
                  onClick={() => {
                ...
                <div className="space-y-6">
                  <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                    <div className="flex items-center space-x-3 mb-3">
                      ...
                      <h3 className="font-bold text-green-900">¡Aplicación creada exitosamente!</h3>
                    </div>
                    <p className="text-green-700 text-sm">
                      Guarda estas credenciales en un lugar seguro. No podrás verlas nuevamente.
                    </p>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <button
                          onClick={() => copyToClipboard(newAppCredentials.apiKey, 'API Key')}
                    ...
                    <div>
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <button
                          onClick={() => copyToClipboard(newAppCredentials.apiSecret, 'API Secret')}
                          className="px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
                        >
                          <FiCopy className="w-4 h-4" />
                        </button>
                    ...
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                      <h4 className="font-medium text-yellow-900 mb-2">Endpoint de Validación</h4>
                      <code className="block text-sm text-yellow-800 bg-yellow-100 p-2 rounded">
                        POST https://api.mundero.mgx.world/api/integraciones/validar
                      </code>
                      <p className="text-yellow-700 text-sm mt-2">
                        Usa este endpoint con el header: Authorization: Bearer {newAppCredentials.apiKey}
                      </p>
                    </div>
                  </div>
