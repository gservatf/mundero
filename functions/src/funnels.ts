...
export const onFunnelSubmissionCreate = functions.firestore
  ...
  .onCreate(async (snap: DocumentSnapshot, context: EventContext) => {
    ...
    try {
      functions.logger.info("Processing funnel submission", {
        submissionId,
        funnelId: submission.funnelId,
      });

      // Get funnel details
      const funnelDoc = await db
        .collection("funnels")
        .doc(submission.funnelId)
        .get();
      if (!funnelDoc.exists) {
        functions.logger.error("Funnel not found", {
          funnelId: submission.funnelId,
        });
        return;
      }
      ...
      functions.logger.info("Funnel submission processed successfully", {
        submissionId,
      });
    } catch (error) {
...
async function runWorkflow(
  ...
  try {
    functions.logger.info("Starting workflow execution", {
      workflowId,
      submissionId,
    });
    ...
    functions.logger.info("Workflow execution completed", {
      workflowId,
      submissionId,
      executionId,
    });
...
async function executeWorkflowNode(
  ...
  try {
    functions.logger.info("Executing workflow node", {
      nodeId: node.id,
      type: node.type,
    });
...
async function handleEmailNode(
  node: WorkflowNode,
  submission: FunnelSubmission,
) {
...
async function handleWebhookNode(
  node: WorkflowNode,
  submission: FunnelSubmission,
) {
  const config = node.data.config;
...
async function handleDelayNode(
  ...
  functions.logger.info("Scheduling delay", {
    duration,
    unit,
    nodeId: node.id,
  });
...
async function handleNotifyHrNode(
  node: WorkflowNode,
  submission: FunnelSubmission,
) {
  const config = node.data.config;
...
async function handleStartSolutionNode(
  node: WorkflowNode,
  submission: FunnelSubmission,
) {
  const config = node.data.config;
...
async function runQuickDestination(submission: FunnelSubmission, funnel: any) {
  functions.logger.info("Running quick destination for submission", {
    submissionId: submission.id,
  });
...
export const onFunnelCreate = functions.firestore
  ...
  .onWrite(async (change: Change<DocumentSnapshot>, context: EventContext) => {
    ...
    try {
      ...
      functions.logger.info("Funnel created successfully", {
        funnelId,
        slug: funnel.slug,
      });
    } catch (error) {
...
async function validateSlugUniqueness(
  ...
  const conflicts = existingFunnels.docs.filter(
    (doc: admin.firestore.DocumentSnapshot) => doc.id !== excludeFunnelId,
  );
...
async function copyOrganizationBranding(
  organizationId: string,
  funnelId: string,
) {
...
export const validateFunnelAccess = functions.https.onCall(
  async (data: any, context: functions.https.CallableContext) => {
    ...
    try {
      ...
      functions.logger.error("Error validating funnel access", {
        funnelId,
        userId,
        error,
      });
...
export const triggerWorkflow = functions.https.onCall(
  async (data: any, context: functions.https.CallableContext) => {
    ...
    try {
      ...
      // Validate access
      const hasAccess = await isOrgOwnerOrAdmin(
      ...
      functions.logger.error("Error triggering workflow", {
        workflowId,
        submissionId,
        userId,
        error,
